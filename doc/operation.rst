...
in case of operator's mistake. In such a case, the safe way to modify the zone
file is to freeze zone events first::
...
causing freeze pending. So we use the blocking mode to be sure that the operation
was finished. Then we can flush the frozen zone contents.

Now we open a text editor and perform desired changes to the zone file.
If "`zonefile-load: difference-no-serial`" is not set, it's necessary to
