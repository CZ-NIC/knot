...
Handling zonefile, journal, changes, serials
============================================

Some configuration options regarding zonefile and journal, together with operation
...
contents in the zonefile, and also modify it (recommended with turned-off Knot or
taking the :ref:`safe way<Editing zonefile>`). Journal serves here just as a source of
history for slaves' IXFR. Some users dislike that Knot overwrites their prettily prepared
zonefile.
...
Zone contents are stored just in journal, changes to the zone done just by DDNS or
knotc. The user might have filled the zone contents initially from zonefile by setting
"zonefile-load: whole" temporarily. It's also a good setup for slaves. Anyway,
it's recommended to carefully tune the journal-size-related options to avoid
surprises of journal getting full.
...
The user can make changes to the zone by editing zonefile, and his pretty zonefile
gets never overwritten and filled with DNSSEC-related autogenerated records - they are
only stored in journal.

The user must take responsibility here, with each change to zonefile, to increment
the SOA serial number to a higher value, than the current SOA serial in the zone
(not in the zonefile)!
...
Achieving the same setup as in previous, here the user does not have to take care about
SOA serials at all, they are incremented with any change to the zone. This would be
obvoiusly a bad idea for behind-anycast servers, because their serials might go out of sync.
