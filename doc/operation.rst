...
Knot DNS stores DNSSEC keys in textual `PEM <https://en.wikipedia.org/wiki/Privacy-Enhanced_Mail>`_
format, while many HSM management software require the keys to be in binary
`DER <https://en.wikipedia.org/wiki/X.690#DER_encoding>`_ format to be imported.
Keys can be converted by software tools such as ``certtol`` from `GnuTLS <https://www.gnutls.org/>`_
suite or ``openssl`` from `OpenSSL <https://www.openssl.org/>`_ suite.
...
Actual import of keys (both public and private keys from the same keypair) to an HSM can be done
via PKCS #11 interface, by ``pkcs11-tool`` from `OpenSC <https://github.com/OpenSC/OpenSC/wiki>`_ toolkit
for example.  In the example below, ``/usr/local/lib/pkcs11.so`` is used as the name of PKCS #11 library
