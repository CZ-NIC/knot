...
and :ref:`maximum depth <zone_journal-max-depth>`. Before hitting any of the limits,
changesets are stored one-by-one and whole history is linear. When hitting any of the limits,
the zone is flushed into the zone file and the oldest changesets are deleted. In the
case of the size limit, twice [#fn-hc]_ the needed amount of space is purged to
prevent too frequent deletes. Further zone file flush is invoked after the journal
runs out of deletables "flushed changesets" if not already synchronized by the flush event.
...
changing same zone records, re-signing whole zone etc. Thus the difference between the zone
