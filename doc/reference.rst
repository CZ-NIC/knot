...
- ``name`` — the owner of each updated RR must match at least one name in the :ref:`update-owner-name<acl_update_owner_name>` list
...
- ``equal`` — the owner of each updated RR must be equal to at least one domain set by :ref:`update-owner<acl_update_owner>`.
...
A list of allowed owners of RRs in a zone update used with :ref:`update-owner<acl_update_owner>` set to ``name``.
