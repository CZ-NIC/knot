...
- ``none`` - No catalog zone.
- ``interpret`` - This zone is loaded from zone file or XFR, and member zones shall be configured based on its contents.
...
.. NOTE::
   This option must be set iff ``catalog-role`` is ``interpret``.
