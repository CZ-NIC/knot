...
#
# usage:
#   ./scripts/make-obs.sh [path.to.archive.xz] [release]
#
# supply archive as optional argument to build from,
...
sed -i 's/^\(Source0:\s\+\).*/\1knot_%{version}.orig.tar.xz/' $OUTDIR/*.spec
: rendering PKGBUILD template
apkg srcpkg $APKG_OPTS -d arch --render-template
: fixing PKGBUILD to use debian source archive
sed -i 's/^source=.*/source=("knot_${pkgver}.orig.tar.xz")/' $OUTDIR/PKGBUILD
popd >/dev/null
