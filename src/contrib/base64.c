...
/*! \brief Transformation and validation table for decoding Base64url. */
...
static int32_t _base64_generic_encode(const uint8_t  *in,
					  ...
					  const uint8_t *alphabet)
{
	...
	while (in < stop) {
		text[0] = alphabet[in[0] >> 2];
		text[1] = alphabet[(in[0] & 0x03) << 4 | in[1] >> 4];
		text[2] = alphabet[(in[1] & 0x0F) << 2 | in[2] >> 6];
		text[3] = alphabet[in[2] & 0x3F];
		text += 4;
		in += 3;
	}
	...
	case 2:
		text[0] = alphabet[in[0] >> 2];
		text[1] = alphabet[(in[0] & 0x03) << 4 | in[1] >> 4];
		text[2] = alphabet[(in[1] & 0x0F) << 2];
		text[3] = alphabet[PD];
		text += 3 + alphabet[PD] && 1;
		break;
	case 1:
		text[0] = alphabet[in[0] >> 2];
		text[1] = alphabet[(in[0] & 0x03) << 4];
		text[2] = alphabet[PD];
		text[3] = alphabet[PD];
		text += 2 + 2 * (alphabet[PD] && 1);
		break;
	}
...
static int32_t _base64_generic_encode_alloc(const uint8_t  *in,
                            const uint32_t in_len,
                            uint8_t        **out,
							const uint8_t *alphabet)
{
	...
	*out = malloc(out_len);
	...
	// Encode data.
	int32_t ret = _base64_generic_encode(in, in_len, *out, out_len, alphabet);
	if (ret < 0) {
		free(*out);
		*out = NULL;
	}
...
int32_t base64_encode(const uint8_t  *in,
                      ...
                      const uint32_t out_len)
{
	return _base64_generic_encode(in, in_len, out, out_len, base64_enc);
}
...
int32_t base64_encode_alloc(const uint8_t  *in,
                            ...
                            uint8_t        **out)
{
	return _base64_generic_encode_alloc(in, in_len, out, base64_enc);
}
...
int32_t base64url_encode(const uint8_t  *in,
                      ...
                      const uint32_t out_len)
{
	return _base64_generic_encode(in, in_len, out, out_len, base64url_enc);
}
...
int32_t base64url_encode_alloc(const uint8_t  *in,
                            ...
                            uint8_t        **out)
{
	return _base64_generic_encode_alloc(in, in_len, out, base64url_enc);
}

static int32_t _base64_generic_decode(const uint8_t  *in,
                      const uint32_t in_len,
                      uint8_t        *out,
                      const uint32_t out_len,
					  const uint8_t *alphabet)
...
static int32_t _base64_generic_decode_alloc(const uint8_t  *in,
                            const uint32_t in_len,
                            uint8_t        **out,
							const uint8_t *alphabet)
{
	...
	*out = malloc(out_len);
	...
	// Decode data.
	int32_t ret = _base64_generic_decode(in, in_len, *out, out_len, alphabet);
	if (ret < 0) {
		free(*out);
		*out = NULL;
	}
...
int32_t base64_decode(const uint8_t  *in,
                      ...
                      const uint32_t out_len)
{
	return _base64_generic_decode(in, in_len, out, out_len, base64_dec);
}
...
int32_t base64_decode_alloc(const uint8_t  *in,
                            ...
                            uint8_t        **out)
{
	return _base64_generic_decode_alloc(in, in_len, out, base64_dec);
}
...
int32_t base64url_decode(const uint8_t  *in,
                      ...
                      const uint32_t out_len)
{
	return _base64_generic_decode(in, in_len, out, out_len, base64url_dec);
}
...
int32_t base64url_decode_alloc(const uint8_t  *in,
                            ...
                            uint8_t        **out)
{
	return _base64_generic_decode_alloc(in, in_len, out, base64url_dec);
}
