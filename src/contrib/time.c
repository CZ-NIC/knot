                      ...
                      size_t max_units, knot_time_t time, long millis, const char *suffix)
{
	...
	*dst++ = (diff < 0 ? '-' : '+');
	...
	ret = snprintf_check(dst, dst_len, "%s", suffix);
	return (ret >= 0 ? 0 : -1);
}
                       ...
                       char *dst, size_t dst_len)
{
	...
	switch (format) {
	case TIME_PRINT_UNIX:
		ret = snprintf_check(dst, dst_len, "%"KNOT_TIME_PRINTF".%03ld%s", time, millis, suffix);
		return (ret > 0 ? 0 : -1);
	case TIME_PRINT_ISO8601:
		if (time > LONG_MAX) {
			return -1;
		...
		if (ret > 0) {
			ret = snprintf_check(dst + ret, dst_len - ret, ".%03ldZ%s", millis, suffix);
		}
		return (ret > 0 ? 0 : -1);
	case TIME_PRINT_ISO8601Z:
		if (time > LONG_MAX) {
	...
	case TIME_PRINT_RELSEC:
		...
		ret = snprintf_check(dst, dst_len, "%+"KNOT_TIMEDIFF_PRINTF".%03ld%s",
		                     d, millis, suffix);
		return (ret > 0 ? 0 : -1);
	case TIME_PRINT_HUMAN_MIXED:
