...
int net_connect(net_t *net)
{
	...
	if (net->socktype == SOCK_STREAM) {
		...
		if (net->quic.params != NULL && net->quic.params->enable) {
			...
			int ret = 0;
			if ((ret = quic_ctx_connect(&net->quic, sockfd, net->srv->ai_addr, net->remote_str)) != KNOT_EOK) {
				close(sockfd);
				return ret;
			}
		}
...
int net_send(const net_t *net, const uint8_t *buf, const size_t buf_len)
{
	...
	if (net->quic.params != NULL && net->quic.params->enable) {
		if (quic_ctx_send((quic_ctx_t *)&net->quic, buf, buf_len) != KNOT_EOK) {
...
int net_receive(const net_t *net, uint8_t *buf, const size_t buf_len)
{
	...
	if (net->quic.params != NULL && net->quic.params->enable) {
		int ret = quic_ctx_receive((quic_ctx_t *)&net->quic, buf, buf_len);
		if (ret < 0) {
