...
#define quic_get_encryption_level(level) \
	ngtcp2_crypto_gnutls_from_gnutls_record_encryption_level(level)
#define quic_send(ctx, sockfd, family) \
	quic_send_data(ctx, sockfd, family, NULL, 0)
...
#define set_application_error(ctx, error_code, reason, reason_len) \
	ngtcp2_ccerr_set_application_error(&(ctx)->last_err, error_code, \
	                                   reason, reason_len)

#define set_transport_error(ctx, error_code, reason, reason_len) \
	ngtcp2_ccerr_set_transport_error(&(ctx)->last_err, error_code, \
	                                 reason, reason_len)
                               ...
                               void *user_data, void *stream_user_data)
{
	...
	if (stream_id != ctx->stream.id) {
		set_application_error(ctx, DOQ_PROTOCOL_ERROR,
		                      (uint8_t *)"Unknown stream",
		                      sizeof("Unknown stream") - 1);
		return NGTCP2_ERR_CALLBACK_FAILURE;
	}

	/**
	 * @note This ACK processing is necessary because of the "knotd"
	 * responses where the ACK framework is missing. In the future, let's
	 * look into the RFC and fix this. The idea is that when the server
	 * responds in the correct stream, assume it has received a complete
	 * query.
	 */
	...
	if (ret != KNOT_EOK) {
		set_application_error(ctx, DOQ_PROTOCOL_ERROR,
		                      (uint8_t *)"Payload malformed",
		                      sizeof("Payload malformed") - 1);
		return NGTCP2_ERR_CALLBACK_FAILURE;
	}
...
int quic_ctx_connect(quic_ctx_t *ctx, int sockfd, struct addrinfo *dst_addr)
{
	...
	params.max_ack_delay = NGTCP2_SECONDS;
