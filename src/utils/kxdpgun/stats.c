...
size_t collect_stats(kxdpgun_stats_t *into, kxdpgun_stats_t **sts)
{
	...
	for (kxdpgun_stats_t **it = sts; *it != NULL; ++it) {
		const kxdpgun_stats_t *what = *it;
 ...
 */
void json_stats_hdr(xdp_gun_ctx_t *xdp_ctx)
{
	xdp_gun_ctx_t *ctx = xdp_ctx;
	jsonw_t *w = ctx->jw;
...
void plain_stats_hdr(xdp_gun_ctx_t *xdp_ctx)
{
	xdp_gun_ctx_t *ctx = xdp_ctx;
...
void plain_thrd_summary(const xdp_gun_ctx_t *ctx, const kxdpgun_stats_t *st)
{
	...
	INFO2("thread#%02u: sent %"PRIu64"%s%s%s", ctx->thread_id, st->qry_sent, recv_str,
	      lost_str, err_str);
}

void plain_stats(kxdpgun_stats_t *st, const xdp_gun_ctx_t *ctx, stats_type_t stt)
{
	pthread_mutex_lock(&st->mutex);
...
void json_stats(kxdpgun_stats_t *st, const xdp_gun_ctx_t *ctx, stats_type_t stt)
{
