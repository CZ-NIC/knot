...
static int read_txt(struct pkt_payload **g_payloads_top_p, FILE *f, txt_bufs_t *bufs, uint16_t edns_size, uint16_t msgid) {
...
static int read_bin(struct pkt_payload **g_payloads_top_p, FILE *f, bin_bufs_t *bufs, uint16_t msgid) {
	assert(g_payloads_top_p != NULL);
	struct pkt_payload *g_payloads_top = *g_payloads_top_p;
	uint16_t size;
	if (fread(&size, sizeof(size), 1, f) < 1) { //TODO validate
		return 0;
	}
	size = ntohs(size);
...
bool load_queries(const input_t *input, uint16_t edns_size, uint16_t msgid, size_t maxcount)
{
	...
	case BIN:
		...
		break;
	default:
		assert(0); // Assert when devel, fail when release
		goto fail;
	}
