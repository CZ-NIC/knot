...
int keymgr_nsec3_salt(kdnssec_ctx_t *ctx, const char *new_salt)
{
	...
	knot_time_t created;
	int ret = kasp_db_load_nsec3salt(*ctx->kasp_db, ctx->zone->dname, &salt_bin, &created);
	switch (ret) {
	case KNOT_EOK:
		printf("Current salt: ");
		if (salt_bin.size == 0) {
	...
	if (ret == KNOT_EOK && new_salt != NULL) {
		...
		if (salt_bin.size != ctx->policy->nsec3_salt_length) {
			printf("Warning: specified salt doesn't match configured salt length (%d).\n",
			       (int)ctx->policy->nsec3_salt_length);
		}
		ret = kasp_db_store_nsec3salt(*ctx->kasp_db, ctx->zone->dname, &salt_bin, created);
		if (salt_bin.size > 0) {
			free(salt_bin.data);
		}
	}
