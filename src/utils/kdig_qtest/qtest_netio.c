                    ...
                    const quic_params_t   *quic_params)
{
	...
	if (https_params != NULL && https_params->enable) {
		int ret = net->cbs->tls_ctx_init(&net->tls, tls_params,
...
int net_connect(net_t *net)
{
	...
	net->cbs->get_addr_str((struct sockaddr_storage *)net->srv->ai_addr,
	...
	if (net->socktype == SOCK_STREAM) {
		...
		if (net->tls.params != NULL) {
			...
			if (net->https.params.enable) {
				...
				char *remote = net->cbs->net_get_remote(net);
				ret = net->cbs->tls_ctx_setup_remote_endpoint(&net->tls, &doh_alpn, 1, NULL,
				        remote);
				if (ret != 0) {
					net_close(net);
					return ret;
...
#endif //LIBNGHTTP2
			{
				...
				ret = net->cbs->tls_ctx_setup_remote_endpoint(&net->tls, &dot_alpn, 1,
				        knot_tls_priority(true), net->cbs->net_get_remote(net));
				if (ret != 0) {
					net_close(net);
					return ret;
				}
		...
		if (net->quic.params.enable) {
			...
			ret = net->cbs->tls_ctx_setup_remote_endpoint(&net->tls,
			        &doq_alpn, 1, knot_tls_priority(false),
				net->cbs->net_get_remote(net));
			if (ret != 0) {
				net_close(net);
				return ret;
			}
			ret = net->cbs->quic_ctx_connect(&net->quic, sockfd,
...
int net_set_local_info(net_t *net)
{
	...
	net->cbs->get_addr_str((struct sockaddr_storage *)net->local_info->ai_addr,
	...
	if (net->socktype == SOCK_DGRAM) {
		...
		while (true) {
			...
			if (from_len > sizeof(from) ||
				...
				char *src = NULL;
				net->cbs->get_addr_str(&from, get_protocol(net), &src);
