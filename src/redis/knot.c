...
static int upd_dump(RedisModuleCtx *ctx, RedisModuleKey *index_key, const arg_dname_t *origin, const arg_dname_t *opt_owner, const uint16_t *opt_rtype, bool txt)
{
	int zone_keytype = RedisModule_KeyType(index_key);
	if (zone_keytype == REDISMODULE_KEYTYPE_EMPTY) {
		RedisModule_CloseKey(index_key);
...
static int upd_load_serial(RedisModuleCtx *ctx, size_t *counter, const arg_dname_t *origin, const rdb_txn_t *txn, const uint32_t serial_final, const uint32_t serial, const arg_dname_t *opt_owner, const uint16_t *opt_rtype, const bool txt)
{
...
static int upd_load(RedisModuleCtx *ctx, const arg_dname_t *origin, rdb_txn_t *txn, const uint32_t serial, const arg_dname_t *opt_owner, const uint16_t *opt_rtype, bool txt)
{
	if (set_active_transaction(ctx, origin, txn) != KNOT_EOK) {
		RedisModule_ReplyWithError(ctx, "ERR not active zone");
		return KNOT_EEXIST;
