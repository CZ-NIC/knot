...
static uint32_t rdb_stream_cleanup_ms = 60000;
...
static RedisModuleString *meta_keyname_construct(const uint8_t prefix, RedisModuleCtx *ctx, const arg_dname_t *origin, uint8_t instance)
{
...
static bool zone_txn_get_when_open(RedisModuleCtx *ctx, const arg_dname_t *origin, const rdb_txn_t *txn, RedisModuleKey **key, int rights)
{
                        ...
                        uint8_t instance, uint32_t serial)
{
	...
	RedisModuleString *events[] = {
		RedisModule_CreateString(ctx, RDB_EVENT_ARG_EVENT, 1),
		RedisModule_CreateStringFromLongLong(ctx, type),
		RedisModule_CreateString(ctx, RDB_EVENT_ARG_ORIGIN, 1),
		RedisModule_CreateString(ctx, (const char *)origin->data, origin->len),
		RedisModule_CreateString(ctx, RDB_EVENT_ARG_INSTANCE, 1),
		RedisModule_CreateStringFromLongLong(ctx, instance),
		RedisModule_CreateString(ctx, RDB_EVENT_ARG_SERIAL, 1),
	...
	if (rdb_stream_cleanup_ms == 0) {
		return KNOT_EOK;
	}

	ts.ms -= rdb_stream_cleanup_ms;
	ts.seq = 0;
...
static RedisModuleKey *find_upd_index(RedisModuleCtx *ctx, const arg_dname_t *origin, const rdb_txn_t *txn, uint16_t id, int rights)
{
...
static RedisModuleString *commited_upd_keyname_construct(RedisModuleCtx *ctx, const arg_dname_t *origin, const uint8_t instance, uint32_t serial)
{
	RedisModule_Assert(ctx != NULL);
...
static RedisModuleKey *find_commited_upd_index(RedisModuleCtx *ctx, const arg_dname_t *origin, const rdb_txn_t *txn, const uint32_t serial, int rights)
{
	RedisModule_Assert(ctx != NULL);
...
static RedisModuleString *rrset_keyname_construct(RedisModuleCtx *ctx, const rdb_txn_t *txn, const arg_dname_t *origin, const arg_dname_t *owner, uint16_t rtype)
{
...
static int zone_meta_active_exchange(RedisModuleCtx *ctx, RedisModuleKey *key, rdb_txn_t *txn, const arg_dname_t *origin)
{
	...
	if (active_old != ZONE_META_INACTIVE) {
		...
		delete_zone_index(ctx, &txn_old, origin);
...
static int zone_commit(RedisModuleCtx *ctx, const arg_dname_t *origin, rdb_txn_t *txn)
{
	...
	return commit_event(ctx, RDB_EVENT_ZONE, origin, txn->instance, 0);
}
...
static int zone_load(RedisModuleCtx *ctx, const arg_dname_t *origin, rdb_txn_t *txn, const arg_dname_t *opt_owner, uint16_t *opt_rtype, bool txt)
{
...
static bool upd_meta_get_when_open(RedisModuleCtx *ctx, const arg_dname_t *origin, const rdb_txn_t *txn, RedisModuleKey **key, int rights)
{
	RedisModule_Assert(key != NULL && *key == NULL);
...
static RedisModuleString *diff_keyname_construct(RedisModuleCtx *ctx, const arg_dname_t *origin, const rdb_txn_t *txn, const arg_dname_t *owner, uint16_t rtype, uint16_t id)
{
	RedisModule_Assert(ctx != NULL && txn != NULL);
...
static int upd_add_rem(RedisModuleCtx *ctx, const arg_dname_t *origin, const rdb_txn_t *txn, const arg_dname_t *owner, const uint32_t ttl, const uint16_t rtype, const knot_rdata_t *rdataset, upd_callback cb)
{
	assert(cb != NULL);
...
static int upd_commit(RedisModuleCtx *ctx, const arg_dname_t *origin, rdb_txn_t *txn)
{
	...
	foreach_in_zset(upd_key) {
		...
		for (size_t i = 0; i < rr_count; ++i) {
			rdata_add(ctx, &zone_txn, origin, &owner, rtype, diff->dest_ttl ? diff->dest_ttl : rdb_default_ttl, diff->add_rrs.rdata);
			rr = knot_rdataset_next(rr);
		}
	...
	if (knot_soa_serial(rrset->rrs.rdata) == serial) {
		...
		diff_v *diff = RedisModule_Calloc(1, sizeof(diff_v));
		knot_rdataset_add(&diff->remove_rrs, rrset->rrs.rdata, &mm);
		knot_soa_serial_set(rrset->rrs.rdata, serial + 1);
		knot_rdataset_add(&diff->add_rrs, rrset->rrs.rdata, &mm);
	...
	return commit_event(ctx, RDB_EVENT_UPD, origin, txn->instance, serial);
}
...
static int upd_diff(RedisModuleCtx *ctx, const arg_dname_t *origin, const rdb_txn_t *txn, const arg_dname_t *opt_owner, uint16_t *opt_rtype, bool txt)
{
	int ret = get_id(ctx, origin, txn);
	if (ret < 0 || ret > UINT16_MAX) {
		RedisModule_ReplyWithError(ctx, "Unknown transaction ID");
...
static int upd_load(RedisModuleCtx *ctx, const arg_dname_t *origin, const rdb_txn_t *txn, uint32_t serial, const arg_dname_t *opt_owner, uint16_t *opt_rtype, bool txt)
{
	RedisModuleKey *index_key = find_commited_upd_index(ctx, origin, txn, serial, REDISMODULE_READ | REDISMODULE_WRITE);
	if (index_key == NULL || RedisModule_KeyType(index_key) != REDISMODULE_KEYTYPE_ZSET) {
		return RedisModule_ReplyWithError(ctx, "ERR Unable find");
	}
...
static int upd_load_txt(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
	...
	return upd_load(ctx, &origin, &txn, serial, (argc >= 5) ? &owner : NULL,
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
	...
	for (int i = 0; i < argc; i += 2) {
		...
		} else if (strncmp(key, "stream-cleanup-ms", key_len) == 0) {
			if (RedisModule_StringToLongLong(argv[i + 1], &num) == REDISMODULE_OK) {
				rdb_stream_cleanup_ms = num;
			} else {
				LOAD_ERROR(ctx, "invalid configuration option value");
			}
		} else {
	...
	RedisModule_Log(ctx, REDISMODULE_LOGLEVEL_NOTICE, "loaded with default-ttl=%u, stream-cleanup-ms=%u", rdb_default_ttl, rdb_stream_cleanup_ms);
