...
static rrset_k rrset_key_construct(RedisModuleCtx *ctx, const rdb_txn_t *txn,
                                   const arg_dname_t *origin,
                                   const arg_dname_t *owner, int16_t rtype,
                                   int rights)
{
	RedisModuleString *rrset_keystr = rrset_keyname_construct(ctx, txn, origin, owner, rtype);
	rrset_k rrset_key = RedisModule_OpenKey(ctx, rrset_keystr, rights);
...
static int rrset_key_set(RedisModuleCtx *ctx, rrset_k key, RedisModuleString *keyname, const arg_dname_t *origin, const rdb_txn_t *txn, uint16_t rtype, rrset_v *val)
{
	...
	index_k zone_index_key = find_zone_index(ctx, origin, txn, REDISMODULE_READ | REDISMODULE_WRITE);
...
static int rrset_key_set_impl(RedisModuleCtx *ctx, rrset_k key, rrset_v *val)
{
	RedisModule_Assert(ctx != NULL && key != NULL && val != NULL);

	RedisModuleString *rrset_keystr = RedisModule_CreateStringFromString(ctx, RedisModule_GetKeyNameFromModuleKey(key));
	size_t key_strlen = 0;
	const char *key_str = RedisModule_StringPtrLen(rrset_keystr, &key_strlen);
	...
	rdb_txn_t txn;
	wire_ctx_read(&w, &txn, sizeof(txn));
	RedisModule_Assert(w.error == KNOT_EOK);

	int ret = rrset_key_set(ctx, key, rrset_keystr, &origin, &txn, rtype, val);
	RedisModule_FreeString(ctx, rrset_keystr);
	return ret;
}

static int rdata_add(RedisModuleCtx *ctx, const arg_dname_t *origin, const rdb_txn_t *txn,
...
static zone_meta_k zone_meta_key_get(RedisModuleCtx *ctx, rdb_txn_t *txn, const arg_dname_t *origin, int rights)
{
	...
	zone_meta_k key = RedisModule_OpenKey(ctx, keyname, rights);
	RedisModule_FreeString(ctx, keyname);
...
static void scanner_data(zs_scanner_t *s)
{
	...
	switch (s_ctx->mode) {
	case STORE:
		ret = rdata_add(s_ctx->ctx, &origin, s_ctx->txn, &owner, s->r_type,
		                s->r_ttl, rdata);
		break;
	case ADD:
		ret = knot_upd_add_txt(s_ctx->ctx, &origin, s_ctx->txn, &owner,
		                       s->r_ttl, s->r_type, rdata);
		break;
	case REM:
		ret = knot_upd_remove_txt(s_ctx->ctx, &origin, s_ctx->txn, &owner,
		                          s->r_ttl, s->r_type, rdata);
		break;
	default:
                                  ...
                                  const size_t zone_data_len)
{
	...
	if (RedisModule_KeyType(rrset_key) != REDISMODULE_KEYTYPE_EMPTY) {
		...
		RedisModule_CloseKey(rrset_key);
		RedisModule_ReplyWithError(ctx, "ERR non-empty RRset");
		return;
	}
	...
	if (rrset == NULL) {
		RedisModule_FreeString(ctx, rrset_keyname);
		RedisModule_CloseKey(rrset_key);
		RedisModule_ReplyWithError(ctx, "ERR failed to allocate memory");
		return;
	}
	...
	if (ret != KNOT_EOK) {
		RedisModule_Free(rrset);
		RedisModule_ReplyWithError(ctx, "ERR unable to store RRset");
		return;
	}
...
static int run_scanner(scanner_ctx_t *s_ctx, const arg_dname_t *origin,
...
static void zone_store_txt_format(RedisModuleCtx *ctx, const arg_dname_t *origin, const rdb_txn_t *txn, const char *zone_data, const size_t zone_data_len)
{
	...
	if (run_scanner(&s_ctx, origin, zone_data, zone_data_len) != KNOT_EOK) {
		return;
	}
	RedisModule_ReplyWithSimpleString(ctx, RDB_RETURN_OK);
}
...
static void zone_commit(RedisModuleCtx *ctx, const arg_dname_t *origin, rdb_txn_t *txn)
{
	...
	if (soa_keyname == NULL) {
		RedisModule_CloseKey(meta_key);
		RedisModule_ReplyWithError(ctx, "ERR missing SOA record");
		return;
	}
...
static void zone_abort(RedisModuleCtx *ctx, const arg_dname_t *origin, rdb_txn_t *txn)
{
	...
	if (ret == KNOT_EOK) {
		...
		RedisModule_ReplyWithError(ctx, "ERR non-existent zone");
	} else {
		RedisModule_ReplyWithError(ctx, "ERR corrupted database");
	}
}

static int zone_exists(RedisModuleCtx *ctx, const arg_dname_t *origin, rdb_txn_t *txn)
{
	int ret = set_active_transaction(ctx, origin, txn);
	if (ret != KNOT_EOK) {
		RedisModule_ReplyWithError(ctx, "ERR unknown instance");
		return KNOT_ERROR;
	}

	RedisModuleKey *zone_index_key = find_zone_index(ctx, origin, txn, REDISMODULE_READ);
	...
	if (zone_keytype == REDISMODULE_KEYTYPE_EMPTY) {
		RedisModule_CloseKey(zone_index_key);
		RedisModule_ReplyWithError(ctx, "ERR does not exist");
		return KNOT_ERROR;
                      ...
                      uint16_t *opt_rtype, dump_mode_t mode)
{
	if (txn->id == TXN_ID_ACTIVE) {
		...
		RedisModule_ReplyWithError(ctx, "ERR unable find");
		return;
	}
	...
	foreach_in_zset (index_key) {
		...
		if (rrset_key == NULL) {
			// TODO when fails, it breaks output
			RedisModule_ReplyWithError(ctx, "ERR corrupted database");
			break;
		}
...
static int upd_commit(RedisModuleCtx *ctx, const arg_dname_t *origin, rdb_txn_t *txn, bool abort_on_fail)
{
	...
	if (RedisModule_ModuleTypeGetType(soa_key) != rdb_rrset_t) {
		RedisModule_ReplyWithError(ctx, "ERR Bad data");
		return KNOT_ENOENT;
	}
	...
	foreach_in_zset(upd_key) {
		...
		for (size_t i = 0; i < rr_count; ++i) {
			rdata_add(ctx, origin, &zone_txn, &owner, rtype, diff->add_ttl, diff->add_rrs.rdata);
			rr = knot_rdataset_next(rr);
		}
                           ...
                           const arg_dname_t *opt_owner, const uint16_t *opt_rtype, const dump_mode_t mode)
{
	...
	if (serial_next != serial_final) {
		upd_load_serial(ctx, counter, origin, txn, serial_final, serial_next, opt_owner, opt_rtype, mode);
	}
                    ...
                    const uint32_t serial, const arg_dname_t *opt_owner, const uint16_t *opt_rtype, dump_mode_t mode)
{
	...
	size_t counter = 0;
	RedisModule_ReplyWithArray(ctx, REDISMODULE_POSTPONED_ARRAY_LEN);
	upd_load_serial(ctx, &counter, origin, txn, serial, serial_it, opt_owner, opt_rtype, mode);
