                     ...
                     const arg_dname_t *owner, int16_t rtype, uint32_t ttl, const knot_rdata_t *rdata)
{
	...
	if (RedisModule_KeyType(rrset_key) == REDISMODULE_KEYTYPE_EMPTY) {
		...
		int ret = RedisModule_ModuleTypeSetValue(rrset_key, rdb_rrset_t, rrset);
		if (ret != REDISMODULE_OK) {
...
static int dump_rrset(RedisModuleCtx *ctx, knot_rrset_t *rrset, char *buf,
                      size_t buf_size, long *count, bool compact)
{
...
static int zone_load(RedisModuleCtx *ctx, const arg_dname_t *origin, rdb_txn_t *txn,
                     const arg_dname_t *opt_owner, uint16_t *opt_rtype, unsigned mode)
{
	...
	foreach_in_zset (index_key) {
		...
		if (mode == 0) {
			...
			if (dump_rrset(ctx, &rrset_out, buf, sizeof(buf), &count, mode == 2) != 0) {
				RedisModule_CloseKey(rrset_key);
				break;
			}
		}
