...
static int send_rrset2(const knot_rrset_t *rrset, void *vctx)
{
	send_ctx_t *ctx = vctx;
...
static int zone_txn_diff_l(zone_t *zone, ctl_args_t *args)
{
	...
	if (ret == KNOT_EOK) {
		...
		ret = zone_update_foreach(zone->control_update, false, send_rrset2, ctx);
	}
	if (ret == KNOT_EOK) {
		ctx->data[KNOT_CTL_IDX_FILTERS] = CTL_FILTER_DIFF_ADD_R;
		ret = zone_update_foreach(zone->control_update, true, send_rrset2, ctx);
	}
