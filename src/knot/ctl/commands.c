...
static int init_backup(ctl_args_t *args, bool restore_mode)
{
	...
	size_t journal_size = knot_lmdb_db_usage(&args->server->journaldb);
	journal_size *= 2; // to make sure that current contents fit into backup DB
	journal_size += 1024 * 1024;

	zone_backup_ctx_t *ctx;

	int ret = zone_backup_init(1, dest, kasp_db_size, conf_int(&timer_db_size), journal_size, &ctx);
	if (ret == KNOT_EOK) {
		assert(ctx != NULL);
