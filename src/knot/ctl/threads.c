...
static int ctl_socket_thr(struct dthread *dt)
{
	...
	ctl_init_ctxs(concurrent_ctxs, CTL_MAX_CONCURRENT, ctx->server, dt->idx * (CTL_MAX_CONCURRENT + 1));
	...
	while (dt->unit->threads[0]->state & ThreadActive) {
		...
		ret = ctl_manage(my_ctl, ctx->server, &this_thread_exclusive, 0, concurrent_ctxs, CTL_MAX_CONCURRENT);
