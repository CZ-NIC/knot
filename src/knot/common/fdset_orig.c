...
/* Realloc memory or return error (part of fdset0_resize). */
#define MEM_RESIZE(tmp, p, n) \
	if ((tmp = realloc((p), (n) * sizeof(*p))) == NULL) \
		return KNOT_ENOMEM; \
	(p) = tmp;

static int fdset0_resize(fdset0_t *set, unsigned size)
{
	void *tmp = NULL;
...
int fdset0_init(fdset0_t *set, unsigned size)
{
	...
	memset(set, 0, sizeof(fdset0_t));
	return fdset0_resize(set, size);
}
...
int fdset0_add(fdset0_t *set, int fd, unsigned events, void *ctx)
{
	...
	/* Realloc needed. */
	if (set->n == set->size && fdset0_resize(set, set->size + FDSET0_INIT_SIZE))
		return KNOT_ENOMEM;
...
int fdset0_sweep(fdset0_t* set, fdset0_sweep_cb_t cb, void *data)
{
	...
	while (i < set->n) {
		...
		if (set->timeout[i] > 0 && set->timeout[i] <= now.tv_sec) {
			if (cb(set, i, data) == FDSET0_SWEEP) {
				if (fdset0_remove(set, i) == KNOT_EOK)
