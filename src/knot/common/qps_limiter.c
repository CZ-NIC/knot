...
int qps_limiter_init(qps_limiter_t *limiter)
{
	...
	memset(limiter->qps_limit, 0, sizeof(qps_limit_t));
...
bool qps_limiter_is_allowed(qps_limiter_t *limiter, time_t time, bool is_err)
{
	...
	/* time_t could be 64bit. Using 64bit time will cause 128bit atomic operations which is not
	   optimal. We can keep time as from start of program to ensure 32bit gives us 68 years of
	   run without restart, and still be 64bit atomic operations. */
