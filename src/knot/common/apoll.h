...
//#define USE_EPOLL 1
	...
	#define apoll_ctx_remove(ctx, it) epoll_ctx_remove_it(ctx, it)
	...
	#define apoll_ctx_poll(ctx, it, offset, ev_size, timeout) epoll_ctx_wait(ctx, it, offset, ev_size, timeout)
	...
	#define apoll_it_ev_is_pollin(it) epoll_it_ev_is_poll(it)
	...
	#define apoll_ctx_remove(ctx, it) fdset_remove_it(ctx, it)
	...
	#define apoll_ctx_poll(ctx, it, offset, ev_size, timeout) fdset_wait(ctx, it, offset, ev_size, timeout)
	...
	#define apoll_it_ev_is_pollin(it) fdset_it_ev_is_poll(it)
