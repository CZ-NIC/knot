...
int server_start_answering(server_t *server)
{
	...
	log_info("server started answering");
...
static int configure_threads(conf_t *conf, server_t *server)
{
	...
	bool use_numa = conf->cache.srv_numa_enabled;
#ifdef KNOT_ENABLE_NUMA
	use_numa = use_numa && (-1 != numa_available());
#endif
	ret = init_udp_async(conf->cache.srv_udp_async_reqs, use_numa);
	if (ret != KNOT_EOK) {
		return ret;
	}

	ret = init_tcp_async(conf->cache.srv_tcp_async_reqs, use_numa);
	if (ret != KNOT_EOK) {
		return ret;
	}
#endif
