...
static iface_t *server_init_xdp_iface(struct sockaddr_storage *addr)
{
#ifndef ENABLE_XDP
	assert(0);
	return NULL;
...
static int configure_sockets(conf_t *conf, server_t *s)
{
	...
	while (lisxdp_val.code == KNOT_EOK) {
		...
		iface_t *new_if = server_init_xdp_iface(&addr);
		if (new_if != NULL) {
			memcpy(&newlist[real_n++], new_if, sizeof(*newlist));
			free(new_if);
		}
...
static bool listen_changed(conf_t *conf, server_t *server)
{
	...
	while (listen_val.code == KNOT_EOK) {
		...
		for (size_t i = 0; i < server->n_ifaces; i++) {
			if (sockaddr_cmp(&addr, &server->ifaces[i].addr) == 0) {
				matches++;
				found = true;
				break;
			}
...
static int configure_threads(conf_t *conf, server_t *server, size_t xdp_threads)
{
	...
	if (xdp_threads > 0) {
		ret = set_handler(server, IO_XDP, xdp_threads, true, udp_master);
		if (ret != KNOT_EOK) {
			return ret;
		}
	}
