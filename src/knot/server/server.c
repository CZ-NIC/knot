                              ...
                              char since[static RDB_TIMESTAMP_SIZE])
{
	...
	conf_val_t val = conf_zone_get(conf(), C_ZONE_DB_IN, origin);
	if (conf_int(&val) != instance) {
		return;
	}
...
static int rdb_listener_run(struct dthread *thread)
{
	...
	while (thread->state == ThreadActive) {
		...
		redisReply *reply = redisCommand(ctx, "XREAD BLOCK %d STREAMS %b %s",
		                                 2000, RDB_EVENT_KEY, strlen(RDB_EVENT_KEY), since);
		if (reply == NULL) {
			sleep(2);
...
static int reconfigure_remote_pool(conf_t *conf, server_t *server)
{
	...
	if (global_redis_pool == NULL && val.code == KNOT_EOK) {
		...
		conn_pool_t *new_pool = conn_pool_init(bg_wrkrs, REDIS_CONN_POOL_TIMEOUT, free_redis_conn, invalid_redis_conn);
		if (new_pool == NULL) {
			return KNOT_ENOMEM;
		}
