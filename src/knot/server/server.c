...
static int init_creds(conf_t *conf, server_t *server)
{
	...
	conf_val_t val = conf_get(conf, C_SERVER, C_CA_FILE);
	while (val.code == KNOT_EOK) {
		const char *ca_file = conf_str(&val);
		if (*ca_file == '\0') {
			system_ca = true;
		} else if ((ret = check_file(ca_file, "ca")) != KNOT_EOK) {
			goto failed;
		} else {
			if (ptrlist_add(&ca_files, (char *)ca_file, NULL) == NULL) {
				ret = KNOT_ENOMEM;
				goto failed;
			}
		}
