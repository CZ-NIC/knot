...
static int configure_sockets(conf_t *conf, server_t *s)
{
	...
	knot_xdp_config_t xdp_config = {
		...
		.busypoll_budget = conf_get_int(conf, C_XDP, C_BUSYPOLL_BUDGET),
		.busypoll_timeout = conf_get_int(conf, C_XDP, C_BUSYPOLL_TIMEOUT),
	 };
...
static void warn_server_reconfigure(conf_t *conf, server_t *server)
{
	...
	static bool warn_ring_size_check = true;
	...
	if (warn_ring_size_check && conf->cache.xdp_ring_size != conf_get_int(conf, C_XDP, C_RING_SIZE)) {
		log_warning(msg, &C_RING_SIZE[1]);
		warn_ring_size_check = false;
	}
