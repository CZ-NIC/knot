...
int uq_send_sweep(struct knot_quic_reply *r)
{
	...
	if (r->ip_loc->ss_family == AF_INET6) {
		...
		memcpy(&((struct in6_pktinfo *)CMSG_DATA(&cmsg.cmsg))->ipi6_addr, &((const struct sockaddr_in6 *)r->ip_loc)->sin6_addr, sizeof(struct in6_addr));
	} else {
		cmsg.cmsg.cmsg_level = IPPROTO_IP;
		cmsg.cmsg.cmsg_type = IP_PKTINFO;
		cmsg.cmsg.cmsg_len = CMSG_LEN(sizeof(struct in_pktinfo));
		memcpy(&((struct in_pktinfo *)CMSG_DATA(&cmsg.cmsg))->ipi_addr, &((const struct sockaddr_in *)r->ip_loc)->sin_addr, sizeof(struct in_addr));
	} // this is simply terrible but it only says "send it with given outgoing IP address"

	struct msghdr msg = {
		.msg_iov = r->out_payload,
		.msg_iovlen = 1,
		.msg_name = (void *)r->ip_rem,
		.msg_namelen = r->ip_rem->ss_family == AF_INET6 ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in),
		.msg_control = &cmsg,
