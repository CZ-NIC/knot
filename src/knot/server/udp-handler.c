...
static void cmsg_handle_ecn(int **p_ecn, struct cmsghdr *cmsg)
{
	if (cmsg->cmsg_level == IPPROTO_IPV6 && cmsg->cmsg_type == IPV6_RECVTCLASS &&
	    cmsg->cmsg_len >= sizeof(**p_ecn)) {
		cmsg->cmsg_type = IPV6_TCLASS; // Change the type for outgoing cmsg.
		*p_ecn = (int *)CMSG_DATA(cmsg);
	}
#ifdef IP_RECVTOS
	else if (cmsg->cmsg_level == IPPROTO_IP && cmsg->cmsg_type == IP_RECVTOS &&
	    cmsg->cmsg_len >= sizeof(**p_ecn)) {
		cmsg->cmsg_type = IP_TOS; // Change the type for outgoing cmsg.
		*p_ecn = (int *)CMSG_DATA(cmsg);
	}
#endif
}
