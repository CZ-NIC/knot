...
static uint32_t zones_soa_timer(zone_t *zone, uint32_t (*rr_func)(const knot_rdataset_t*))
{
	...
	soa_rrs = node_rdataset(zc->apex, KNOT_RRTYPE_SOA);
	assert(soa_rrs != NULL);
	ret = rr_func(soa_rrs);
...
int zones_zonefile_sync(zone_t *zone, journal_t *journal)
{
	...
	/* Latest zone serial. */
	const knot_rdataset_t *soa_rrs = node_rdataset(contents->apex,
	                                          KNOT_RRTYPE_SOA);
	assert(soa_rrs != NULL);
...
int zones_journal_apply(zone_t *zone)
{
	...
	soa_rrs = node_rdataset(contents->apex, KNOT_RRTYPE_SOA);
	assert(soa_rrs != NULL);
	int64_t serial_ret = knot_soa_serial(soa_rrs);
	if (serial_ret < 0) {
		rcu_read_unlock();
