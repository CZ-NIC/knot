...
static int try_ds(const knot_dname_t *zone_name, const conf_remote_t *parent, zone_key_t *key, size_t timeout, uint32_t *ds_ttl)
{
...
static bool parents_have_ds(kdnssec_ctx_t *kctx, zone_key_t *key, size_t timeout, uint32_t *max_ds_ttl)
{
	bool success = false;
	uint32_t ds_ttl;
