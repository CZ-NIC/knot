                                ...
                                nsec_chain_iterate_data_t *data)
{
	...
	ret = zone_update_remove(data->update, &aorig, true);
	if (ret == KNOT_EOK) {
		ret = zone_update_add(data->update, acopy, true);
	}
	knot_rrset_free(acopy, NULL);
	return ret;
}
                              ...
                              uint32_t ttl, bool opt_out, const knot_dname_t *for_node)
{
	...
	if (old_nsec3_n != NULL) {
		...
		if (!knot_rrset_empty(&rem_nsec3)) {
			...
			if (1) {
				ret = zone_update_remove(update, &rem_nsec3, true);
				if (ret == KNOT_EOK && !knot_rrset_empty(&rem_rrsig)) {
					ret = zone_update_remove(update, &rem_rrsig, true);
				}
			}
	...
	if (add_nsec3 && ret == KNOT_EOK) {
		...
		if (ret == KNOT_EOK) {
			ret = zone_update_add(update, &nsec3, true);
		}
