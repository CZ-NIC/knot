...
int knot_dnssec_key_rollover(kdnssec_ctx_t *ctx, zone_sign_roll_flags_t flags, zone_sign_reschedule_t *reschedule)
{
	...
	if ((flags & KEY_ROLL_FORCE_KSK_ROLL) && ret == KNOT_EOK) {
		...
		if (running_rollover(ctx)) {
			log_zone_warning(ctx->zone->dname, "DNSSEC, forced KSK rollover impossible due to running rollover");
		} else {
	...
	if ((flags & KEY_ROLL_FORCE_ZSK_ROLL) && ret == KNOT_EOK) {
		...
		if (running_rollover(ctx)) {
			log_zone_warning(ctx->zone->dname, "DNSSEC, forced ZSK rollover impossible due to running rollover");
		} else {
