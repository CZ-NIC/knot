...
int knot_dnssec_validate_zone(zone_update_t *update, validation_conf_t *val_conf)
{
	...
	if (!val_conf->fast) { // useful when re-checking unchanged state for expired RRSIGs. Also, RRSIGs' crypto is skipped thanks to NODE_FLAGS_RRSIGS_VALID, only expiration time is checked.
