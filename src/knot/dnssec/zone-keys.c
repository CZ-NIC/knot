...
static bool keytag_in_use(kdnssec_ctx_t *ctx, uint16_t keytag)
{
	for (size_t i = 0; i < ctx->zone->num_keys; i++) {
		uint16_t confl = dnssec_key_get_keytag(ctx->zone->keys[i].key);
		if (confl == keytag) {
			return true;
		}
	}
                                      ...
                                      dnssec_key_t **key)
{
	...
	log_zone_notice(ctx->zone->dname, "generated key with keytag conflict %hu",
			 ...
			 knot_kasp_key_t **key_ptr)
{
	...
	int r = generate_keytag_unconflict(ctx, size, flags, &id, &dnskey);
	if (r != KNOT_EOK) {
		return r;
	}
