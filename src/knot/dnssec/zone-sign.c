                          ...
                          knot_time_t *expires_at)
{
	...
	*expires_at = knot_time_add(dnssec_ctx->now, dnssec_ctx->policy->rrsig_lifetime);
	...
	for (size_t i = 0; i < num_threads; i++) {
		args[i].tree = tree;
		args[i].zone_keys = zone_keys;
		args[i].dnssec_ctx = dnssec_ctx;
	...
	for (size_t i = 0; i < num_threads; i++) {
		log_zone_info(update->zone->name, "thread init errcode: %d", args[i].thread_init_errcode);
                   ...
                   knot_time_t *expire_at)
{
	...
	result = zone_tree_sign(update->new_cont->nodes, 8, zone_keys, dnssec_ctx, update, &normal_expire);
	if (result != KNOT_EOK) {
		return result;
	}

	knot_time_t nsec3_expire = 0;
	result = zone_tree_sign(update->new_cont->nsec3_nodes, 8, zone_keys, dnssec_ctx,
				update, &nsec3_expire);
	if (result != KNOT_EOK) {
		return result;
	}
