                   ...
                   knot_time_t *expire_at)
{
	...
	result = zone_tree_sign(update->new_cont->nodes, dnssec_ctx->policy->signing_workers,
	...
	result = zone_tree_sign(update->new_cont->nsec3_nodes, dnssec_ctx->policy->signing_workers,
                          ...
                          knot_time_t *expire_at)
{
	...
	for (size_t i = 0; i < num_threads; i++) {
		...
		changeset_iter_all(&args[i].itt, &update->change);
		args[i].sign_ctx = zone_sign_ctx(zone_keys, dnssec_ctx);
		(void)changeset_init(&args[i].changeset, update->zone->name);
		args[i].expires_at = 0;
		args[i].num_threads = num_threads;
	...
	if (num_threads == 1) {
		...
		// start working threads
		for (size_t i = 0; i < num_threads; i++) {
			args[i].thread_init_errcode = pthread_create(&args[i].thread, NULL, sign_changeset_thread, &args[i]);
		}
