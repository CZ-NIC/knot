...
#define CHECK_RET if (ret != KNOT_EOK) goto cleanup
	...
	if (dnssec_ctx->policy->ds_push && node_rrtype_exists(ch.add->apex, KNOT_RRTYPE_CDS)) {
		...
		time_t next_ds_push = time(NULL) + dnssec_ctx->policy->propagation_delay;
		update->zone->timers.next_ds_push = next_ds_push;
		zone_events_schedule_at(update->zone, ZONE_EVENT_DS_PUSH, next_ds_push);
	}
