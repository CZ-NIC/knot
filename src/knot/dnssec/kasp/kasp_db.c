...
int kasp_db_delete_keys(knot_lmdb_db_t *db, const knot_dname_t *zone_name, bool best)
{
	kdnssec_ctx_t ctx = { 0 };
	...
	knot_lmdb_foreach(&txn, &prefix) {
		...
		assert(count > 0);
		if (count == 1) {
			ret = kdnssec_delete_from_keystores(ctx.keystores, key_id, true);
...
int kasp_db_sweep_keys(knot_lmdb_db_t *db, sweep_cb keep_zone, void *cb_data)
{
	...
	ret = zone_init_keystore(conf(), &null_policy_id, NULL, &keystores);
	if (ret != KNOT_EOK) {
		return ret;
	}
