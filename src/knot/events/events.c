...
<<<<<<< HEAD
			pthread_cond_broadcast(events->blocking_cv[type]);
			events->blocking_cv[type] = NULL;
=======
			pthread_cond_broadcast(events->blocking_cv);
			events->blocking_cv = NULL;
>>>>>>> d401f4e67... blocking tasks: Proof of concept (WiP)
		}
...
void zone_events_schedule_blocking(zone_t *zone, zone_event_type_t type, bool user)
{
	...
	while (zone->events.blocking_cv[type]) { //TODO blokujici event bezi.. vymyslet (domyslet), jak se zachovat...
		pthread_cond_wait(zone->events.blocking_cv[type], &mutex_blocker);
	}
	...
	while (zone->events.blocking_cv[type]) { // 'spurious wakeup' condition
		pthread_cond_wait(&cv , &mutex_blocker); 
	}
