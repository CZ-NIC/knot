...
static void finalize_timers_base(struct refresh_data *data, bool also_expire)
{
	...
	SipHash24_Update(&ctx, zone->name, knot_dname_size(zone->name));
	uint64_t random = SipHash24_End(&ctx);
	zone->timers->next_refresh -= conf_jitter(jitter, percent, random,
	                                          soa_refresh, soa_refresh -1);
                            ...
                            bool axfr_bootstrap)
{
	...
	REFRESH_LOG(LOG_INFO, data,
	            ...
	            (data->zone->contents->nodes->flags & ZONE_TREE_DELEG_AWARE) ? ", DELEG-aware" : "",
	            expires_in);
}
