...
int event_load(conf_t *conf, zone_t *zone)
{
	...
	if (zf_from != ZONEFILE_LOAD_NONE) {
		...
		if (zf_conts != NULL && zf_from == ZONEFILE_LOAD_DIFSE && relevant != NULL) {
			...
			log_zone_info(zone->name, "zonefile parsed, serial corrected %u -> %u", zone->zonefile.serial, set);
			zone->zonefile.serial = set;
		} else {
			log_zone_info(zone->name, "zonefile parsed, serial %u", zone->zonefile.serial);
		}
