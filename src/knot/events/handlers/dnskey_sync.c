...
static uint16_t dnskey_keytag(knot_rdata_t *rd, const knot_dname_t *owner)
{
	uint16_t res = 0;
	const dnssec_binary_t bin = {
	        .size = rd->len, .data = rd->data
	};
	return dnssec_keytag(&bin, &res) == 0 ? res : 0;
}


static void filter_keytag(knot_rrset_t *rr, uint32_t keytag_remain, uint32_t keytag_modulo, const char *msg)
{
	for (int i = 0; i < rr->rrs.count; i++) { // NOTE that count may decrease during iteration
		knot_rdata_t *rd = knot_rdataset_at(&rr->rrs, i);
		uint16_t rd_keytag = rr->type == KNOT_RRTYPE_CDS ? knot_ds_key_tag(rd) : dnskey_keytag(rd, rr->owner);
...
static int compute_rem_add(struct dnskey_sync_data *data, int idx)
{
	...
	filter_keytag(data->add_rr[idx], data->keytag_remain, data->keytag_modulo, "add");
...
static int dnskey_sync_consume(knot_layer_t *layer, knot_pkt_t *pkt)
{
	...
	filter_keytag(data->rem_rr[idx], data->keytag_remain, data->keytag_modulo, "rem");
                            ...
                            conf_val_t *dnssec_policy)
{
	...
	int zero, ret = serial_modulo_parse(conf_str(&val), &data.keytag_remain, &data.keytag_modulo, &zero);
	if (ret != KNOT_EOK) {
		return ret;
	}
...
int event_dnskey_sync(conf_t *conf, zone_t *zone)
{
	...
	while (iter.id->code == KNOT_EOK) {
		...
		for (int i = 0; i < addr_count; i++) {
			...
			int ret = send_dnskey_sync(conf, zone, &uptodate, &parent, timeout, &policy_id);
			if (ret == KNOT_EOK) {
				break;
			}
		}
