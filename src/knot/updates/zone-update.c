...
static bool soa_differs_just_serial(const knot_rrset_t *soa1, const knot_rrset_t *soa2)
{
...
static int diff_just_ser_cb(const knot_rrset_t *rr, void *ctx)
{
	bool ignore_zonemd = *((bool *)ctx);
	...
	for (int i = 0; i < rr->rrs.count; i++) {
		uint16_t type = knot_rrsig_type_covered(rd);
		if (type != KNOT_RRTYPE_SOA && (!ignore_zonemd || type != KNOT_RRTYPE_ZONEMD)) {
			return 1;
		}
...
static int ch_rem_cb(const knot_rrset_t *rr, void *ctx)
{
	return changeset_add_removal(ctx, rr, 0);
}

static int ch_add_cb(const knot_rrset_t *rr, void *ctx)
{
