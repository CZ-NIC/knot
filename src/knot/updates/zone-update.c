...
static int dump_rrset(const knot_rrset_t *rr, void *vctx)
{
	dump_changeset_ctx_t *ctx = vctx;
...
int zone_update_external(conf_t *conf, zone_update_t *update, conf_val_t *ev_id)
{
	...
	log_zone_error(update->zone->name, "waiting for external validation");
...
int zone_update_commit(conf_t *conf, zone_update_t *update)
{
	...
	conf_val_t val = conf_zone_get(conf, C_EXTERNAL_VLDT, update->zone->name);
	if (val.code == KNOT_EOK) {
...
static int trees_foreach(zone_tree_t *nodes, zone_tree_t *nsec3ns, bool subtract_counterparts,
                         rrset_cb_t cb, void *ctx)
{
	zone_tree_it_t it = { 0 };
	int ret = zone_tree_it_double_begin(nodes, nsec3ns, &it);
