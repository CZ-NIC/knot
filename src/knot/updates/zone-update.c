...
int zone_update_remove(zone_update_t *update, const knot_rrset_t *rrset)
{
	...
	if (update->flags & UPDATE_INCREMENTAL) {
		...
		rrset_copy->ttl = node_rrset_ttl(n, rrset->ttl);
...
{
	conf_val_t val = conf_zone_get(conf, C_ZONE_DB_OUT, update->zone->name);
...
int zone_update_commit(conf_t *conf, zone_update_t *update)
{
	...
	if (ret != KNOT_EOK) {
		log_zone_error(update->zone->name, "zone DB update failed (%s)", knot_strerror(ret));
		discard_adds_tree(update);
		return ret;
	}
