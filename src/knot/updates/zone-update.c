                     ...
                     zone_update_flags_t flags)
{
	...
	if (zone->control_update != NULL && zone->control_update != update) {
		log_zone_warning(zone->name, "open ctl zone transaction leads to blocking zone update");
	}
                              ...
                              zone_contents_t *new_cont, zone_update_flags_t flags)
{
	...
	if (zone_without_contents->control_update != NULL) {
		log_zone_warning(zone_without_contents->name, "open ctl zone transaction leads to blocking zone update");
	}
