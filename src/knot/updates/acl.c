                      ...
                      const knot_dname_t *zone_name, knot_pkt_t *query)
{
	...
	conf_val_t names = conf_id_get(conf, C_ACL, C_UPDATE_NAME, acl);
	conf_val_t types = conf_id_get(conf, C_ACL, C_UPDATE_TYPE, acl);
	...
	for (uint16_t i = pos; i < pos + count; i++) {
		...
		if (!check_owner_name(rr->owner, name, names)) {
			return false;
		}
	}
	return true;
