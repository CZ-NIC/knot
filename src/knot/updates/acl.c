...
static bool compare_dnames(const knot_dname_t *rr_owner, const knot_dname_t *name,
...
static bool check_owner_name(knot_dname_t *rr_owner,
                             conf_val_t *names, acl_update_owner_match_t match)
{
	...
	while (names->code == KNOT_EOK) {
		if (compare_dnames(rr_owner, conf_dname(names), match)) {
			return true;
		}
		conf_val_next(names);
	}
                         ...
                         const knot_dname_t *zone_name, knot_pkt_t *query)
{
	...
	for (uint16_t i = pos; i < pos + count; i++) {
		...
		bool ret;
		switch (owner) {
		case ACL_UPDATE_OWNER_NAME: ret = check_owner_name(rr->owner, names, match); break;
		case ACL_UPDATE_OWNER_KEY:  ret = compare_dnames(rr->owner, key_name, match); break;
		case ACL_UPDATE_OWNER_ZONE: ret = compare_dnames(rr->owner, zone_name, match); break;
