                 ...
                 knot_tsig_key_t *tsig)
{
	...
	while (rmts->code == KNOT_EOK) {
		conf_val_t val = conf_id_get(conf, C_RMT, C_AUTO_ACL, rmts);
		if (!conf_bool(&val)) {
			goto next_remote;
		}

		conf_val_t key_id = conf_id_get(conf, C_RMT, C_KEY, rmts);
		...
		val = conf_id_get(conf, C_RMT, C_ADDR, rmts);
