...
static knotd_in_state_t follow_cname(knot_pkt_t *pkt, uint16_t rrtype, knotd_qdata_t *qdata)
{
	...
	zone_t *zone = process_query_zone_find(knot_cname_name(cname_rr.rrs.rdata), knot_pkt_qtype(pkt),
	                                       knot_pkt_has_deleg_aware(pkt), server->zone_db);
	if (zone != qdata->extra->zone) {
		qdata->extra->node = NULL; /* Act as if the name leads to nowhere. */
		return KNOTD_IN_STATE_HIT;
	}
