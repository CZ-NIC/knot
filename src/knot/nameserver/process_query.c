...
static zone_t *answer_zone_find_base(struct query_plan *plan, knotd_qdata_t *qdata, knot_pkt_t *query,
                                     const knot_dname_t *qname, uint16_t qtype, uint16_t qclass,
...
{
	return answer_zone_find_base(plan, qdata, query,
	                             knot_pkt_qname(query), knot_pkt_qtype(query),
...
{
	return answer_zone_find_base(NULL, NULL, NULL, qname, qtype,
	                             KNOT_CLASS_IN, KNOTD_QUERY_TYPE_NORMAL,
	                             deleg_aware, zonedb);
}
...
static int answer_edns_put(knot_pkt_t *resp, knotd_qdata_t *qdata)
{
	...
	if (knot_pkt_edns_option(qdata->query, KNOT_EDNS_OPTION_EXPIRE) != NULL &&
		...
		knot_time_t expire = knot_time_min(ATOMIC_GET(qdata->extra->contents->dnssec_expire),
		                                   qdata->extra->zone->timers_static->next_expire);
		uint32_t timer;
		if (expire == 0) {
