...
static int answer_edns_put(knot_pkt_t *resp, knotd_qdata_t *qdata)
{
	...
	/* Add EXPIRE if space. */
	if (knot_pkt_edns_option(qdata->query, KNOT_EDNS_OPTION_EXPIRE) != NULL && qdata->extra->contents != NULL) {
		int32_t timer = qdata->extra->zone->timers.next_expire == 0
		              ? zone_soa_expire(qdata->extra->zone)
		              : qdata->extra->zone->timers.next_expire - time(NULL);
		timer = MAX(timer, 0);
		uint32_t timer_be = htobe32((uint32_t)timer);
		...
		if (knot_pkt_reserve(resp, expire_size) == KNOT_EOK) {
			...
			ret = knot_edns_add_option(&qdata->opt_rr, KNOT_EDNS_OPTION_EXPIRE,
			                           sizeof(timer_be), (uint8_t *)&timer_be, qdata->mm);
			if (ret != KNOT_EOK) {
				return ret;
			}
		}
