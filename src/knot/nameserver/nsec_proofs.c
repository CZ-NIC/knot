...
{
	return node_nsec3_node(node, NULL) == NULL && node->flags & NODE_FLAGS_DELEG;
}
                                      ...
                                      knot_pkt_t *resp)
{
	...
	int ret = put_nsec3_from_node(node_nsec3_node(cpe, NULL), qdata, resp);
	if (ret !=  KNOT_EOK) {
		return ret;
	}
                           ...
                           knot_pkt_t *resp)
{
	...
	zone_node_t *nsec3_match = node_nsec3_node(match, NULL);
	if (nsec3_match != NULL) {
