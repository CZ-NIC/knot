...
int print_migration(FILE *file, int lmdb_major, int lmdb_minor, int lmdb_patch,
                    char *migration_time)
{
	return fprintf(file,
	               "%s%d.%d.%d\n"
	               "migrated_time: %s\n"
	               "migrate_knot_version: %s\n",
	               label_file_lmdb, lmdb_major, lmdb_minor, lmdb_patch,
	               migration_time, PACKAGE_VERSION);
}
...
int backupdir_migrate_label(zone_backup_ctx_t *ctx)
{
	...
	while (ret == KNOT_EOK && knot_getline(&line, &line_size, src) != -1) {
		if (!strncmp(line, label_file_lmdb, sizeof(LABEL_FILE_LMDB) - 1)) {
			found = true;
			ret = fprintf(tmp, "original_%s", line);
			if (ret == KNOT_EOK) {
				ret = print_migration(tmp, lmdb_major, lmdb_minor, lmdb_patch,
				                      migration_time);
			}
		} else {
	...
	if (ret == KNOT_EOK && !found) {
		// For backups made by releases from v3.1.0 and before v3.4.0.
		ret = print_migration(tmp, lmdb_major, lmdb_minor, lmdb_patch,
		                      migration_time);
	}
