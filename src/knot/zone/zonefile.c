...
static void knot_redis_tls_net_close(redisContext *ctx) {
    if (ctx && ctx->fd != REDIS_INVALID_FD) {
	tls_ctx_close(ctx->privctx);
        close(ctx->fd);
...
static void knot_redis_tls_free(void *privctx) {
	tls_ctx_t *tls_ctx = privctx;
	tls_ctx_deinit(tls_ctx);
	hi_free(tls_ctx);
}

static ssize_t knot_redis_tls_read(struct redisContext *ctx, char *buff, size_t size)
{
	int ret = tls_ctx_receive((tls_ctx_t *)ctx->privctx, (uint8_t *)buff, size);
	if (ret > 0) {
		return ret;
	} else if (ret == 0) {
		return -1;
...
{
	//TODO just test - debugging breakpoint, not working
	redis_tls_ctx_t *priv_ctx = (redis_tls_ctx_t *)ctx->privctx;

	size_t len = priv_ctx->last_len ? priv_ctx->last_len : sdslen(ctx->obuf);
	int ret = tls_ctx_send(&priv_ctx->tls, (uint8_t *)ctx->obuf, sdslen(ctx->obuf));
	if (ret > 0) {
		priv_ctx->last_len = 0;
	} else if (ret < 0) {
		priv_ctx->last_len = len;
		return 0;
	}
	return len;
}

static ssize_t knot_redis_tls_async_read(struct redisAsyncContext *async)
{
	//TODO just test - debugging breakpoint, not working
	redisContext *ctx = &async->c;
	// return tls_ctx_receive((tls_ctx_t *)ctx->privctx, (uint8_t *)buff, size);
	return 0UL;
}

static ssize_t knot_redis_tls_async_write(struct redisAsyncContext *async)
{
	redisContext *ctx = &async->c;
	redis_tls_ctx_t *priv_ctx = (redis_tls_ctx_t *)ctx->privctx;
...
redisContextFuncs redisContextGnuTLSFuncs = {
    .close = knot_redis_tls_net_close,
    .free_privctx = knot_redis_tls_free,
    .async_read = knot_redis_tls_async_read,
    .async_write = knot_redis_tls_async_write,
    .read = knot_redis_tls_read,
    .write = knot_redis_tls_write
};

static redisContext *redisConnectWithGnuTLS(const char *path, const struct timeval tv) {
	...
	redis_tls_ctx_t *priv_ctx = (redis_tls_ctx_t *)hi_calloc(1, sizeof(redis_tls_ctx_t));
	tls_params_t params;
	...
	tls_ctx_init(&priv_ctx->tls, &params, 0, tv.tv_sec);
	struct sockaddr_storage ss = { 0 };
	socklen_t ss_len =  sizeof(ss);
	int ret = getsockname(c->fd, (struct sockaddr *)&ss, &ss_len);

	ret = gnutls_set_default_priority_append(priv_ctx->tls.session, "-VERS-ALL:+VERS-TLS1.2:+VERS-TLS1.3", NULL, 0);

	tls_ctx_connect(&priv_ctx->tls, c->fd, false, &ss);
...
redisContext *zone_rdb_connect(conf_t *conf)
{
	...
	redisContext *rdb;
	rdb = redisConnectWithGnuTLS(addr_str, timeout);
...
static int rdb_load(zloader_t *loader)
{
	...
	for (size_t i = 0; i < reply->elements; i++) {
		...
		int ret = process_rdb_data(loader->contents, data);
		if (ret != KNOT_EOK) {
