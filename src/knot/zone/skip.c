...
static uint16_t dnssec_types[] = {
        ...
        KNOT_RRTYPE_NSEC, KNOT_RRTYPE_NSEC3,
        KNOT_RRTYPE_NSEC3PARAM,
        KNOT_RRTYPE_CDS, KNOT_RRTYPE_CDNSKEY,
        0
};
...
static int skip_add_string(zone_skip_t *skip, const char *type_s)
{
	if (strncasecmp(type_s, "dnssec", 7) == 0) {
		return skip_add_dnssec(skip);
	} else {
		uint16_t type = 0;
		if (knot_rrtype_from_string(type_s, &type) > -1) {
...
{
	int ret = skip_add_string(skip, type_s);
