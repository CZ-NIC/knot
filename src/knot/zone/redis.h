...
typedef struct zone_redis_txn {
	...
	int ret; // just helper variable, not set by following functions!
} zone_redis_txn_t;
 ...
 *
 * \param txn           Transaction context structure to be filled (initialize with {0} !).
 ...
 *
 * \retval KNOT_ENOENT   Zone doesn't exist in this instance.
 * \return KNOT_E*
 */
 ...
 * \param log_err     Output: error message in case of failure.
 *
 * \return KNOT_E*
 */
