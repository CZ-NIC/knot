...
static void *adjust_tree_thread(void *vctx)
{
	zone_adjust_arg_t *ctx = vctx;

	ctx->ret = zone_tree_apply(ctx->tree, adjust_single, vctx);
                         ...
                         zone_tree_t *add_changed)
{
	...
	if (threads > 1) {
		assert(nodes_cb != adjust_cb_flags); // This cb demands parent to be adjusted before child => required sequential adjusting (also true for adjust_cb_flags_and_nsec3) !!
		assert(!measure_zone);
		assert(!adjust_prevs);
		if (nsec3_cb != NULL) {
...
int zone_adjust_incremental_update(zone_update_t *update, unsigned threads)
{
	...
	ret = zone_adjust_contents(update->new_cont, adjust_cb_flags, adjust_cb_nsec3_flags, false, true, 1, update->a_ctx->adjust_ptrs);
	if (ret == KNOT_EOK) {
		if (nsec3change) {
	...
	if (ret == KNOT_EOK) {
		if (nsec3change) {
			log_zone_info(update->zone->name, "START adjust update nsec3pointers %u threads", threads);
			...
			log_zone_info(update->zone->name, "FINISH adjust update nsec3pointers %u threads (%s)", threads, knot_strerror(ret));
		} else {
			ret = additionals_reverse_apply_multi(
				update->new_cont->adds_tree,
