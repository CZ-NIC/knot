...
static int remove_rrsig_ignore_rdata(const knot_rrset_t *rrset, knot_rdataset_t *node_rrs, uint16_t covered_type, zone_node_t *node, knot_mm_t *mm)
{
	if (rrset->type != KNOT_RRTYPE_RRSIG || !rrsig_covers_type(rrset, covered_type)) {
		return KNOT_EOK;
	}
	knot_rrset_t covered = node_rrset(node, covered_type), rrsig = node_rrset(node, KNOT_RRTYPE_RRSIG);
	if (knot_rrset_empty(&covered) || !rrsig_covers_type(&rrsig, covered_type)) {
		return KNOT_EOK;
	}
	knot_rrset_t rrsig_soa_rr = create_empty_rrsigs_for(&covered), rrsig_soa_node = create_empty_rrsigs_for(&covered);
	knot_rdataset_t rrsig_nosoa_rr = { 0 };
	int ret = knot_synth_rrsig(covered_type, &rrset->rrs, &rrsig_soa_rr.rrs, NULL);
	assert(ret == KNOT_EOK);
	if (ret == KNOT_EOK) {
		ret = knot_synth_rrsig(covered_type, node_rrs, &rrsig_soa_node.rrs, NULL);
		assert(ret == KNOT_EOK);
	}
	if (ret == KNOT_EOK) {
...
int node_remove_rrset(zone_node_t *node, const knot_rrset_t *rrset, knot_mm_t *mm)
{
	...
	int ret = remove_rrsig_ignore_rdata(rrset, node_rrs, KNOT_RRTYPE_SOA, node, mm);
	if (ret == KNOT_EOK) {
		ret = remove_rrsig_ignore_rdata(rrset, node_rrs, KNOT_RRTYPE_NSEC, node, mm);
	}
	if (ret == KNOT_EOK) {
		ret = remove_rrsig_ignore_rdata(rrset, node_rrs, KNOT_RRTYPE_NSEC3, node, mm);
	}
	if (ret == KNOT_EOK) {
		ret = knot_rdataset_subtract(node_rrs, &rrset->rrs, mm);
	}
	if (ret < 0) {
		return ret;
	}
