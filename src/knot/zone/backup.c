...
const backup_filter_list_t backup_filters[] = {
	{ "zonefile", -1, BACKUP_PARAM_ZONEFILE, CTL_FILTER_BACKUP_ZONEFILE, CTL_FILTER_BACKUP_NOZONEFILE },
	{ "journal",   0, BACKUP_PARAM_JOURNAL,  CTL_FILTER_BACKUP_JOURNAL,  CTL_FILTER_BACKUP_NOJOURNAL },
	{ "timers",    1, BACKUP_PARAM_TIMERS,   CTL_FILTER_BACKUP_TIMERS,   CTL_FILTER_BACKUP_NOTIMERS },
	{ "kaspdb",    2, BACKUP_PARAM_KASPDB,   CTL_FILTER_BACKUP_KASPDB,   CTL_FILTER_BACKUP_NOKASPDB },
	{ "keysonly",  2, BACKUP_PARAM_KEYSONLY, CTL_FILTER_BACKUP_KEYSONLY, CTL_FILTER_BACKUP_NOKEYSONLY },
	{ "catalog",   3, BACKUP_PARAM_CATALOG,  CTL_FILTER_BACKUP_CATALOG,  CTL_FILTER_BACKUP_NOCATALOG },
	{ "quic",     -1, BACKUP_PARAM_QUIC,     CTL_FILTER_BACKUP_QUIC,     CTL_FILTER_BACKUP_NOQUIC },
	{ NULL },
};
...
int zone_backup(conf_t *conf, zone_t *zone)
{
	...
	if (ctx->backup_params & BACKUP_PARAM_TIMERS) {
		...
		if (ctx->restore_mode) {
			...
			ret = zone_timers_write(&ctx->bck_timer_db, zone->name, zone->timers, true);
		}
		if (ret != KNOT_EOK) {
