...
static int apply_one_cb(bool remove, const knot_rrset_t *rr, void *ctx)
{
	...
	if (ret == KNOT_ENOENT && remove && rr->type == KNOT_RRTYPE_RRSIG) {
		// Journal history can contain a changeset with removed RRSIGs
		// which are not present in the zonefile due to zf_load=difference.
		return KNOT_EOK;
	} else {
		return ret;
	}
