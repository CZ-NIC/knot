...
static knot_dname_t *read_dname_with_id(FILE *f)
{
	...
	ret->size = dname_size;
	debug_zload("loaded: dname length: %u\n", ret->size);
                                         ...
                                         int use_ids)
{
	...
	debug_zload("Reading %d items\n", rdata_count);

	debug_zload("current type: %s\n", knot_rrtype_to_string(type));
...
//					printf("%p %s\n", id_array[dname_id]->node,
	//				       knot_dname_to_str(id_array[dname_id]));
		//			getchar();
			...
			debug_zload("read raw_data len %d\n", raw_data_length);
		}
	}
	...
	debug_zload("knot_load_rdata: all %d items read\n",
			 desc->length);
                                         ...
                                         int use_ids)
{
	...
	debug_zload("loading %d rdata entries\n", rdata_count);
                                         ...
                                         int use_ids)
{
	...
	debug_zload("Finished loading RRSet %p\n", rrset);
...
static knot_node_t *knot_load_node(FILE *f, knot_dname_t **id_array)
{
	...
	debug_zload("Node owner id: %d\n", dname_id);
	debug_zload("Node owned by: %s\n", knot_dname_to_str(owner));
	debug_zload("Number of RRSets in a node: %d\n", rrset_count);
...
int knot_zload_open(zloader_t **dst, const char *filename)
{
	*dst = 0;
	...
	if (fscanf(f_crc, "%lu\n", &crc_from_file) != 1) {
		debug_zload("knot_zload_open: could not read "
		                   "CRC from file '%s'\n",
		                   crc_path);
		fclose(f_crc);
		fclose(f);
	...
	if (crc_from_file != crc_calculated) {
		debug_zload("knot_zload_open: CRC failed for "
		                   "file '%s'\n",
		                   filename);
		fclose(f);
		return KNOT_ECRC;
	...
	if (!knot_check_magic(f, MAGIC, MAGIC_LENGTH)) {
		fclose(f);
		debug_zload("knot_zload_open: magic bytes "
				   "in don't match '%*s' "
			 "(%s)\n",
	...
	if (!fread_wrapper(&sflen, 1, sizeof(uint32_t), f)) {
		debug_zload("knot_zload_open: failed to read "
				   "sfile length\n");
		fclose(f);
		return KNOT_ERROR;
	}
	...
	if (!sfile) {
		debug_zload("knot_zload_open: invalid sfile "
				   "length %u\n", sflen);
		fclose(f);
		return KNOT_ENOMEM;
	}
	if (!fread_wrapper(sfile, 1, sflen, f)) {
		debug_zload("knot_zload_open: failed to read %uB "
				   "source file\n",
			 sflen);
		free(sfile);
		fclose(f);
	...
	debug_zload("knot_zload_open: opened '%s' as fp %p "
			   "(source is '%s')\n",
		 filename, f, sfile);
...
{
	debug_zload("Loading zone, loader: %p\n", loader);
	if (!loader) {
		debug_zload("NULL loader!\n");
		return NULL;
	}
...
//	}
	...
	debug_zload("authoritative nodes: %u\n", auth_node_count);

	debug_zload("loading %u nodes\n", node_count);
	...
	debug_zload("total dname count: %d\n", total_dnames);
	...
	debug_zload("Apex node loaded: %p\n", apex);

	knot_zone_t *zone = knot_zone_new(apex, auth_node_count, 0);
	if (zone == NULL) {
		cleanup_id_array(id_array, 1,
				 node_count + nsec3_node_count + 1);
		debug_zload("Failed to create new zone from apex!\n");
		return NULL;
	}
...
//			tmp_node->prev = last_node;
	...
	debug_zload("loading %u nsec3 nodes\n", nsec3_node_count);
...
//		nsec3_first->prev = last_node;
	...
	debug_zload("zone loaded, returning: %p\n", zone);
	return zone;
}
