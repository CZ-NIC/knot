 ...
 */
inline static bool zone_skip_type(zone_skip_t *skip, uint16_t type) { return skip != NULL && rrtype_dynarray_bsearch(skip, &type) != NULL; }

/*!
 * \brief Free any potentially allocated memory by zone_skip structure.
 */
inline static void zone_skip_free(zone_skip_t *skip) { rrtype_dynarray_free(skip); }
