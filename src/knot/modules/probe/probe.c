...
const yp_item_t probe_conf[] = {
	{ MOD_PREFIX,      YP_TSTR, YP_VSTR = { "/tmp/knot-probe-" },           YP_FNONE, { check_prefix } },
	{ NULL }
};
...
int probe_load(knotd_mod_t *mod)
{
	...
	char *prefix = strdup(mod_conf.single.string);
	...
	for (knot_probe_channel_wo_t *it = p->probes; it < &p->probes[p->probe_count]; ++it) {
		if (unlikely((ret = knot_probe_channel_wo_init(it, prefix, (it - p->probes))) != KNOT_EOK)) {
