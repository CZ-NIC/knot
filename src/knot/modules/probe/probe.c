                              ...
                              knotd_qdata_t *qdata, knotd_mod_t *mod)
{
	...
	d.reply.is_present = (state == KNOTD_STATE_NOOP);

	if (pkt->opt_rr) {
		d.edns.is_present = 1;
		d.edns.payload = knot_edns_get_payload(pkt->opt_rr);
		d.edns.version = knot_edns_get_version(pkt->opt_rr);
		d.edns.rcode = knot_edns_get_ext_rcode(pkt->opt_rr);
		d.edns.flags = knot_edns_get_flags(pkt->opt_rr);
		d.edns.options = 0;
