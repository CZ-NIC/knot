                                     ...
                                     knotd_qdata_t *qdata, knotd_mod_t *mod)
{
	...
	if (!(qtype == KNOT_RRTYPE_CDS || qtype == KNOT_RRTYPE_CDNSKEY)) {
		qdata->rcode = KNOT_RCODE_NOERROR;  // other type might exist
	...
	if (memcmp(qdata->name, prefix, prefix_len) != 0) {
		qdata->rcode = KNOT_RCODE_NOERROR;  // may be ENT
		return KNOTD_IN_STATE_NODATA;
	}

	// Copy target zone name
	knot_dname_t target[255];
	unsigned name_len = strlen((const char*)qdata->name) - strlen((const char*)mod->zone) - prefix_len;
