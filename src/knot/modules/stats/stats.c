                                     ...
                                     knotd_qdata_t *qdata, knotd_mod_t *mod)
{
	...
	if (stats->req_bytes) {
		...
		case OPERATION_QUERY:
			knotd_mod_stats_incr(mod, qdata->params->thread_id,
			                     CTR_REQ_BYTES, REQ_BYTES_QUERY,
			                     knot_pkt_size(qdata->query));
			break;
		case OPERATION_UPDATE:
			knotd_mod_stats_incr(mod, qdata->params->thread_id,
			                     CTR_REQ_BYTES, REQ_BYTES_UPDATE,
			                     knot_pkt_size(qdata->query));
			break;
		default:
			if (xfr_packets <= 1) {
				knotd_mod_stats_incr(mod, qdata->params->thread_id,
				                     CTR_REQ_BYTES, REQ_BYTES_OTHER,
				                     knot_pkt_size(qdata->query));
			}
			break;
		}
	...
	if (stats->resp_bytes && state != KNOTD_STATE_NOOP) {
		...
		case OPERATION_QUERY:
			knotd_mod_stats_incr(mod, qdata->params->thread_id,
			                     CTR_RESP_BYTES, RESP_BYTES_REPLY,
			                     knot_pkt_size(pkt));
			break;
		case OPERATION_AXFR:
		case OPERATION_IXFR:
			knotd_mod_stats_incr(mod, qdata->params->thread_id,
			                     CTR_RESP_BYTES, RESP_BYTES_TRANSFER,
			                     knot_pkt_size(pkt));
			break;
		default:
			knotd_mod_stats_incr(mod, qdata->params->thread_id,
			                     CTR_RESP_BYTES, RESP_BYTES_OTHER,
			                     knot_pkt_size(pkt));
			break;
		}
	}
	...
	if (stats->rcode && state != KNOTD_STATE_NOOP) {
		if (xfr_packets <= 1 || rcode != KNOT_RCODE_NOERROR) {
			if (xfr_packets > 1) {
				...
				knotd_mod_stats_decr(mod, qdata->params->thread_id, CTR_RCODE,
				                     KNOT_RCODE_NOERROR, 1);
			}

			if (qdata->rcode_tsig == KNOT_RCODE_BADSIG) {
				knotd_mod_stats_incr(mod, qdata->params->thread_id,
				                     CTR_RCODE, RCODE_BADSIG, 1);
			} else {
				knotd_mod_stats_incr(mod, qdata->params->thread_id,
				                     CTR_RCODE, MIN(rcode, RCODE_OTHER), 1);
			}
		}
	}
	...
	if (stats->protocol) {
		...
		if (qdata->params->remote->ss_family == AF_INET) {
			if (qdata->params->flags & KNOTD_QUERY_FLAG_LIMIT_SIZE) {
				if (xdp) {
					knotd_mod_stats_incr(mod, qdata->params->thread_id,
					                     CTR_PROTOCOL, PROTOCOL_UDP4_XDP, 1);
				} else {
					knotd_mod_stats_incr(mod, qdata->params->thread_id,
					                     CTR_PROTOCOL, PROTOCOL_UDP4, 1);
				}
			} else {
				knotd_mod_stats_incr(mod, qdata->params->thread_id,
				                     CTR_PROTOCOL, PROTOCOL_TCP4, 1);
			}
		} else {
			if (qdata->params->flags & KNOTD_QUERY_FLAG_LIMIT_SIZE) {
				if (xdp) {
					knotd_mod_stats_incr(mod, qdata->params->thread_id,
					                     CTR_PROTOCOL, PROTOCOL_UDP6_XDP, 1);
				} else {
					knotd_mod_stats_incr(mod, qdata->params->thread_id,
					                     CTR_PROTOCOL, PROTOCOL_UDP6, 1);
				}
			} else {
				knotd_mod_stats_incr(mod, qdata->params->thread_id,
				                     CTR_PROTOCOL, PROTOCOL_TCP6, 1);
			}
		}
	}
	...
	// Count EDNS options.
	if (stats->req_eopt) {
		incr_edns_option(mod, qdata->params->thread_id, qdata->query, CTR_REQ_EOPT);
	}
	if (stats->resp_eopt) {
		incr_edns_option(mod, qdata->params->thread_id, pkt, CTR_RESP_EOPT);
	}
	...
	if (stats->nodata && rcode == KNOT_RCODE_NOERROR && state != KNOTD_STATE_NOOP &&
		...
		case KNOT_RRTYPE_A:
			knotd_mod_stats_incr(mod, qdata->params->thread_id,
			                     CTR_NODATA, NODATA_A, 1);
			break;
		case KNOT_RRTYPE_AAAA:
			knotd_mod_stats_incr(mod, qdata->params->thread_id,
			                     CTR_NODATA, NODATA_AAAA, 1);
			break;
		default:
			knotd_mod_stats_incr(mod, qdata->params->thread_id,
			                     CTR_NODATA, NODATA_OTHER, 1);
			break;
		}
	}
	...
	if (stats->qtype) {
		...
		knotd_mod_stats_incr(mod, qdata->params->thread_id, CTR_QTYPE, idx, 1);
	}
	...
	if (stats->qsize) {
		...
		knotd_mod_stats_incr(mod, qdata->params->thread_id,
		                     CTR_QSIZE, MIN(idx, QSIZE_MAX_IDX), 1);
	}
	...
	if (stats->rsize && state != KNOTD_STATE_NOOP) {
		...
		knotd_mod_stats_incr(mod, qdata->params->thread_id,
		                     CTR_RSIZE, MIN(idx, RSIZE_MAX_IDX), 1);
	}
