...
int online_sign_load(knotd_mod_t *mod)
{
	...
	// Create the array for RR types forced into NSEC bitmap.
	conf = knotd_conf_mod(mod, MOD_NSEC_BITMAP);
	if (conf.count == 0) {
		// Use the default list.
		ctx->nsec_force_types = calloc(sizeof(NSEC_FORCE_TYPES)/sizeof(uint16_t), sizeof(uint16_t));
		...
		ctx->nsec_force_types = calloc(conf.count + 1, sizeof(uint16_t));
		if (ctx->nsec_force_types == NULL) {
			knotd_conf_free(&conf);
			return KNOT_ENOMEM;
		}
		for (int i = 0; i < conf.count; i++) {
			ret = knot_rrtype_from_string(conf.multi[i].string, &ctx->nsec_force_types[i]);
			if (ret != KNOT_EOK) {
				knotd_mod_log(mod, LOG_ERR, "failed to parse resource record type (%s)", knot_strerror(ret));
				knotd_conf_free(&conf);
				return ret;
			}
		}
