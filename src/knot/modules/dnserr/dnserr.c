...
static uint64_t log_tuple_hash(const log_tuple_t *val)
{
	...
	return h[0] ^ h[1] ^ h[2]; // Fast hash by XOR qtype, error and first 20 chars of record
}
...
static int hashset_add(log_set_t *set, const log_tuple_t *val)
{
	 ...
	 * A larger number of iterations is a waste of CPU time and has no real
	 * impact on HIT/MISS (except for disproportionately large tables, where
	 * a lot of iterations help).
	 */
...
static void flush_set(knotd_mod_t *mod, log_set_t *set)
{
	...
	char qtype_str[32]; // NOTE should be enough for rrtype
	for (log_tuple_t *el = set->table; el != set->table + set->size; ++el) {
		...
		knotd_mod_log(mod, LOG_WARNING,
		    "client reported error with errno %d for record '%s' with type '%s'",
		    el->err, record_str, qtype_str);
		el->err = 0;
	}
}
...

{
	...
	if (ctx->report_channel != NULL) {
		...
		wire_ctx_t wire = wire_ctx_init(option, ctx->report_channel_size);
		wire_ctx_write(&wire, ctx->report_channel, ctx->report_channel_size);
	}
	...
	if (ctx->log) {
		...
		knot_rrset_t *rr = knot_rrset_new(qdata->name, KNOT_RRTYPE_TXT,
		                                  KNOT_CLASS_IN, 0, &pkt->mm);
		if (rr == NULL) {
			return KNOTD_IN_STATE_ERROR;
		}
		if (knot_rrset_add_rdata(rr, (uint8_t *)"", 1, &pkt->mm) != KNOT_EOK) {
		...
		// NOTE BIND používá wildcard v zóně, aby definovali odpověď
