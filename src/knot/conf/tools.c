	...
	knotd_conf_check_args_t *args)
{
	...
	if (zsk_life_val != 0) { // 0 ~ infinity!
		if (jitter_percent) {
			zsk_life_val -= zsk_life_val * jitter_val / 100;
		} else {
			zsk_life_val -= jitter_val;
		}
		if (zsk_life_val <= 0) {
	...
	if (dnskey_ttl_val == YP_NIL) {
		dnskey_ttl_val = zone_max_ttl_dflt();
	}
	...
	if (jitter_percent) {
		refresh_val += refresh_val * jitter_val / 100;
	} else {
		refresh_val += jitter_val;
	}
	if (refresh_val < zone_max_ttl_val + prop_del_val) {
