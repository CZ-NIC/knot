	...
	bool reinit_cache)
{
	...
	if (first_init || reinit_cache) {
		...
		running_zero_copy = conf_get_bool(conf, C_XDP, C_ZERO_COPY);
	...
	conf->cache.xdp_zero_copy = running_zero_copy;
	...
	conf_flag_t flags)
{
	...
	conf_t *out = calloc(1, sizeof(*out));
	if (out == NULL) {
		return KNOT_ENOMEM;
	}
