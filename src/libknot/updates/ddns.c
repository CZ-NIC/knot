                                      ...
                                      uint16_t type_covered)
{
	...
	if (ret != KNOT_EOK) {
		dbg_xfrin("Failed to add new RRSet to list: %s.\n",
		          knot_strerror(ret));
		knot_rrset_deep_free(&rr_copy, 1, 1, 1);
		knot_rrset_deep_free(&covered_rrset, 1, 1, 1);
		return ret;
                                        ...
                                        knot_changes_t *changes)
{
	...
	knot_rrset_t *rrsigs_old = knot_rrset_get_rrsigs(
	                        node_rrset_copy);
	int ret = 0;
	...
	if (rrsigs_old != NULL) {
		...
		ret = knot_rrset_set_rrsigs(node_rrset_copy, 
		                            rrsigs_copy);
		if (ret != KNOT_EOK) {
...
static int knot_ddnd_process_rem_rrsigs(knot_node_t *node, 
                                        knot_changes_t *changes, 
                                        knot_rrset_t ***removed,
                                        int *removed_count)
{
	...
	*removed_count = 0;
	...
	for (int i = 0; i < rrset_count; ++i) {
		/* Copy RRSet. */
		ret = xfrin_copy_old_rrset(rrsets[i], &rrset_copy, changes);
		if (ret != KNOT_EOK) {
                                       ...
                                       knot_changes_t *changes)
{
	...
	if (type == KNOT_RRTYPE_RRSIG) {
		...
		ret = knot_ddnd_process_rem_rrsigs(node, changes, &removed,
		                                   &removed_count);
	} else {
...
static int knot_ddns_process_rem_all(knot_node_t *node,
                                     ...
                                     knot_changes_t *changes)
{
	/*! \todo Although SOA and NSs are unaffected, their RRSIGs should
	 *        be deleted!
	 */
