                                 ...
                                 unsigned io_timeout_ms)
{
	...
	res->server = server;
	res->creds = creds;
	res->handshake_timeout_ms = GNUTLS_DEFAULT_HANDSHAKE_TIMEOUT;
...
static ssize_t pull_func(gnutls_transport_ptr_t ptr, void *buf, size_t size)
{
	knot_tls_conn_t *conn = (knot_tls_conn_t *)ptr;
	conn->recv_count++;
	ssize_t ret = net_stream_recv(conn->fd, buf, size, conn->timeout);
	if (ret < 0) {
		conn->err_count++;
		conn->last_err = ret;
	}
...
static ssize_t push_func(gnutls_transport_ptr_t ptr, const void *buf, size_t size)
{
	knot_tls_conn_t *conn = (knot_tls_conn_t *)ptr;
	conn->send_count++;
	ssize_t ret = net_stream_send(conn->fd, buf, size, conn->timeout);
	if (ret < 0) {
		conn->err_count++;
		conn->last_err = ret;
	}
...
knot_tls_conn_t *knot_tls_conn_new(knot_tls_ctx_t *ctx, int sock_fd)
{
	...
	int ret = knot_quic_conn_session(&res->session, ctx->creds, TLS_PRIORITIES, "dot", false, ctx->server);
	if (ret != GNUTLS_E_SUCCESS) {
		goto fail;
	}

	gnutls_rnd(GNUTLS_RND_NONCE, &res->conntag, sizeof(res->conntag));
