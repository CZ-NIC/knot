 ...
 * \retval <0 not a subdomain (KNOT_ERANGE) or another error (KNOT_EINVAL).
 */
static inline
int knot_dname_in_bailiwick(const knot_dname_t *domain, const knot_dname_t *bail)
{
	...
	if (label_diff < 0) return KNOT_ERANGE;
	for (int i = 0; i < label_diff; ++i) {
		domain = knot_wire_next_label(domain, NULL);
	}
	return knot_dname_is_equal(domain, bail) ? label_diff : KNOT_ERANGE;
}
