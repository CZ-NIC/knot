...
const size_t XDP_FRAME_PAYLOAD_OFFSET4 = offsetof(struct udpv4, data) + offsetof(struct umem_frame, udpv4);
const size_t XDP_FRAME_PAYLOAD_OFFSET6 = offsetof(struct udpv6, data) + offsetof(struct umem_frame, udpv6);
                        ...
                        const knot_xdp_msg_t *in_reply_to)
{
	...
	size_t ofs = ipv6 ? XDP_FRAME_PAYLOAD_OFFSET6 : XDP_FRAME_PAYLOAD_OFFSET4;
                               ...
                               bool ipv6)
{
	...
	intptr_t pd = (uint8_t *)msg->payload.iov_base - uframe_p
	              - (ipv6 ? XDP_FRAME_PAYLOAD_OFFSET6 : XDP_FRAME_PAYLOAD_OFFSET4);
                             ...
                             uint32_t index)
{
	...
	*xsk_ring_prod__tx_desc(&socket->tx, index) = (struct xdp_desc){
		...
		.len = XDP_FRAME_PAYLOAD_OFFSET4 + msg->payload.iov_len
	};
}
                             ...
                             uint32_t index)
{
	...
	*xsk_ring_prod__tx_desc(&socket->tx, index) = (struct xdp_desc){
		.addr = h->bytes - socket->umem->frames->bytes,
		.len = XDP_FRAME_PAYLOAD_OFFSET6 + msg->payload.iov_len
	};
}
