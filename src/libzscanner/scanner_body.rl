...
%%{
	...
	type_num_ :=
	    ...
	    | "DELEGPARAM"i %{ type_num(KNOT_RRTYPE_DELEGPARAM, &rdata_tail); }
	...
	type_bit =
	    ...
	    | "DELEGPARAM"i %{ window_add_bit(KNOT_RRTYPE_DELEGPARAM, s); }
	...
	# BEGIN - SvcParams and DelegInfos processing (SVCB/HTTPS and DELEG/DELEGPARAM records)
	...
	deleg_key_include  = ("include-delegparam"  %_write16_4);
	...
	deleg_mandat_va = alpha ${ fhold; fcall deleg_mandat_value_; };
	...
	r_type =
		...
		| "DELEGPARAM"i %{ s->r_type = KNOT_RRTYPE_DELEGPARAM; }
		| "TYPE"i      . type_number
		) $!_r_type_error;
	# END
