...
t = Test(address=4, tls=True, tsig=False)
...
try:
    ...
    isset(master.log_search_count(r'ACL, allowed.*TLS$') == 1, "TLS in ACL log")
    isset(master.log_search_count(r'ACL, denied.*TLS/0-RTT') == 0, "TLS/0-RTT in ACL log")
    resp_check(master, "example.com", "AXFR", ticket_no=2, Ortt=True, rcode="NOTAUTH")
    isset(master.log_search_count(r'ACL, allowed.*TLS$') == 1, "TLS in ACL log")
    isset(master.log_search_count(r'ACL, denied.*TLS/0-RTT') == 1, "TLS/0-RTT in ACL log")
finally:
