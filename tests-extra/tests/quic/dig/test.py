...
def query_test(knot, bind, dnssec):
    ...
    # Negative (REFUSED)
    resp = knot.dig("another.world", "SOA", quic=True, dnssec=dnssec)
    resp.check(rcode="REFUSED", flags="QR", noflags="AA TC AD RA")
    resp.cmp(bind)

    # Negative (NXDOMAIN)
    resp = knot.dig("nxdomain.flags", "A", quic=True, dnssec=dnssec)
    ...
    # Positive (SOA)
    resp = knot.dig("flags", "SOA", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR", flags="QR AA", noflags="TC AD RA")
    resp.cmp(bind)

    # Positive (DATA)
    resp = knot.dig("dns1.flags", "A", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR", flags="QR AA", noflags="TC AD RA")
    resp.cmp(bind)

    # Positive (NODATA)
    resp = knot.dig("dns1.flags", "TXT", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR", flags="QR AA", noflags="TC AD RA")
    resp.cmp(bind)

    # Positive (REFERRAL)
    resp = knot.dig("sub.flags", "NS", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR", flags="QR", noflags="AA TC AD RA")
    resp.cmp(bind, additional=True)

    # Positive (REFERRAL, below delegation)
    resp = knot.dig("ns.sub.flags", "A", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR", flags="QR", noflags="AA TC AD RA")
    resp.cmp(bind, additional=True)

    # Positive (REFERRAL, below delegation, ignoring empty-nonterminal during lookup)
    resp = knot.dig("bellow.ns.sub.flags", "A", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR", flags="QR", noflags="AA TC AD RA")
    resp.cmp(bind, additional=True)

    # Positive (NODATA, at delegation, DS type)
    resp = knot.dig("ds-sub.flags", "DS", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR", flags="QR AA", noflags="TC AD RA")
    resp.cmp(bind, additional=True)

    # Positive (NODATA, at delegation, DS type, below empty-non-terminal)
    resp = knot.dig("deleg.ent.flags", "DS", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR", flags="QR AA", noflags="TC AD RA")
    resp.cmp(bind, additional=True)

    # Positive (REFERRAL, below delegation, DS type)
    resp = knot.dig("net.ds-sub.flags", "DS", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR", flags="QR", noflags="AA TC AD RA")
    resp.cmp(bind, additional=True)
    ...
    # CNAME query
    resp = knot.dig("cname.flags", "CNAME", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # CNAME leading to A
    resp = knot.dig("cname.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # CNAME leading to A (NODATA)
    resp = knot.dig("cname.flags", "TXT", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # CNAME leading to delegation
    resp = knot.dig("cname-ns.flags", "NS", quic=True, dnssec=dnssec)
    resp.cmp(bind, additional=True)

    # CNAME leading below delegation
    resp = knot.dig("cname-below-ns.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind, additional=True)

    # CNAME being below a delegation
    resp = knot.dig("cname.below.sub.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind, additional=True)

    # CNAME leading out
    resp = knot.dig("cname-out.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # CNAME leading to wildcard-covered name
    resp = knot.dig("cname-wildcard.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # CNAME leading to wildcard-covered name (NODATA)
    resp = knot.dig("cname-wildcard.flags", "TXT", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # CNAME leading to DNAME tree
    resp = knot.dig("cname-dname.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # CNAME leading to DNAME tree (NXDOMAIN)
    resp = knot.dig("cname-dname-nx.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # CNAME leading to DNAME tree (NODATA)
    resp = knot.dig("cname-dname.flags", "TXT", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Long CNAME loop (Bind truncates the loop at 17 records)
    resp = knot.dig("ab.flags", "A", quic=True, dnssec=dnssec)
    ...
    resp = knot.dig("cname-mx.flags", "MX", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Leading to delegation
    resp = knot.dig("cname-mx-deleg.flags", "MX", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Leading to wildcard-covered name
    resp = knot.dig("cname-mx-wc.flags", "MX", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Leading to name outside zone
    resp = knot.dig("cname-mx-out.flags", "MX", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    ''' DNAME answers. '''

    # DNAME query (NODATA)
    resp = knot.dig("dname.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # DNAME query (NXDOMAIN)
    resp = knot.dig("nxd.dname-dangl.flags", "A", quic=True, dnssec=dnssec)
    resp.check(rcode="NXDOMAIN")
    resp.cmp(bind)

    # CNAME type query on DNAME
    resp = knot.dig("nxd.dname-dangl.flags", "CNAME", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR")
    #resp.cmp(bind) NOTE: this does not work well on Bind (yet)

    # DNAME type query
    resp = knot.dig("dname.flags", "DNAME", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # DNAME being below a delegation
    resp = knot.dig("a.dname.below.sub.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind, additional=True)

    # DNAME query leading out of zone
    resp = knot.dig("a.dname-out.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # DNAME subtree query leading to A
    resp = knot.dig("a.dname.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # DNAME subtree query leading to NODATA
    resp = knot.dig("a.dname.flags", "TXT", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # DNAME subtree query leading to CNAME
    resp = knot.dig("c.dname.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # DNAME subtree query leading to CNAME leading to wildcard
    resp = knot.dig("d.dname.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # DNAME-CNAME-DNAME loop
    resp = knot.dig("e.dname.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # DNAME-DNAME loop
    resp = knot.dig("x.f.dname.flags", "A", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR")
    ...
    # Infinite DNAME loop
    resp = knot.dig("end.dname-outloop.flags", "A", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR")
    ...
    resp = knot.dig("loop.loop.loop.loop.loop.loop.dname-inloop.flags", "A", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR")
    ...
    resp = knot.dig("loop.loop.loop.loop.loop.loop.loop.dname-inloop.flags", "A", quic=True, dnssec=dnssec)
    resp.check(rcode="NOERROR")
    resp.check_record(name="loop.dname-inloop.flags.", rtype="DNAME", ttl=3600, rdata="dname-inloop.flags.")
    compare(resp.count(rtype="CNAME", section="answer"), 5, "Count of synthesized CNAME records in loop.")
    resp.check_counts(7 if dnssec else 6, 0, 0)
    ...
    # Wildcard query
    resp = knot.dig("wildcard.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Wildcard leading to A
    resp = knot.dig("a.wildcard.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Wildcard leading to A (NODATA)
    resp = knot.dig("a.wildcard.flags", "TXT", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Deeper wildcard usage
    resp = knot.dig("a.a.a.wildcard.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Asterisk label
    resp = knot.dig("sub.*.wildcard.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Asterisk label (NODATA)
    resp = knot.dig("sub.*.wildcard.flags", "TXT", quic=True, dnssec=dnssec)
    resp.cmp(bind)
    ...
    resp = knot.dig("*.*.wildcard.flags", "TXT", quic=True, dnssec=dnssec)
    resp.cmp(bind)
    ...
    resp = knot.dig("a.wildcard.dname.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Wildcard under DNAME subtree (NODATA)
    resp = knot.dig("a.wildcard.dname.flags", "TXT", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Wildcard chain to A
    resp = knot.dig("a.wildcard-cname.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Wildcard chain to A (NODATA)
    resp = knot.dig("a.wildcard-cname.flags", "TXT", quic=True, dnssec=dnssec)
    resp.cmp(bind, additional=True)

    # Wildcard chain to NS
    resp = knot.dig("a.wildcard-deleg.flags", "NS", quic=True, dnssec=dnssec)
    ...
    resp = knot.dig("*.a.wildcard-cname.flags", "A", quic=True)
    resp.cmp(bind)

    # Double wildcard expansion
    resp = knot.dig("wild-cname.flags", "TXT", quic=True)
    resp.cmp(bind)

    # Wildcard leading out
    resp = knot.dig("a.wildcard-out.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Wildcard leading to CNAME loop
    resp = knot.dig("test.loop-entry.flags", "A", quic=True, dnssec=dnssec)
    resp.cmp(bind)

    # Wildcard-covered additional record discovery
    resp = knot.dig("mx-additional.flags", "MX", quic=True, dnssec=dnssec)
    resp.cmp(bind)
