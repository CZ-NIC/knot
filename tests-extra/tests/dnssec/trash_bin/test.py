...
isset(len(trash) != 0, "some keys have been deleted")
isset(len(keys) != 0, "not all keys have been deleted")
...
if not master.check_gc_interval(master):
    ...
    for key_id in cur_trash_mod:
        ...
        isset(key_id not in new_kst_keys, f"key {key_id} removed from trash bin keystore")

    # Check that 'trash-touch' works - step two.
    isset(zsk_touched in new_trash, f"touched key {key_id} kept in trash bin")
    isset(zsk_touched in new_kst_keys, f"touched key {key_id} kept in trash bin keystore")
