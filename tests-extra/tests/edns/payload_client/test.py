...
resp = knot.dig("513resp.flags", "TXT", protocol=Protocol.UDP, tsig=False)
resp.check(flags="TC")
resp.cmp(bind, additional=True)

# no TC - UDP message is extended using EDNS0/payload (no TSIG).
resp = knot.dig("513resp.flags", "TXT", protocol=Protocol.UDP, bufsize=524, tsig=False)
resp.check(noflags="TC")
resp.cmp(bind, additional=True)

# TC - UDP message is extended using EDNS0/payload (with TSIG).
resp = knot.dig("513resp.flags", "TXT", protocol=Protocol.UDP, bufsize=524+61, tsig=key)
resp.check(flags="TC")
resp.cmp(bind, additional=True)

# no TC - UDP message is extended using EDNS0/payload (with TSIG).
resp = knot.dig("513resp.flags", "TXT", protocol=Protocol.UDP, bufsize=524+62, tsig=key)
resp.check(noflags="TC")
resp.cmp(bind, additional=True)
