...
def check_zf_types(fname, types):
    with open(fname, "r") as f:
        for t in types:
            compare(f.readline().split()[2], t, "DIFF TYPE %s" % t)
...
slave.zones[ZONE].external = { "timeout": "10", "new": dump_file(slave, "new"), "rem": dump_file(slave, "diff"), "add": dump_file(slave, "diff") }
