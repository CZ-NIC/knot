...
class Server(object):
    ...
    def ctl(self, cmd, availability=True, wait=False):
        ...
        args = self.ctl_params + [self.control_wait] if wait else None + cmd.split()
        try:
...
class Bind(Server):
    ...
    def flush(self, zone=None, wait=False):
        ...
        self.ctl("sync%s" % zone_name, wait)
...
class Knot(Server):
    ...
    def flush(self, zone=None, wait=False):
        ...
        if zone:
            self.ctl("%szone-flush %s" % (params, zone.name), wait)
        else:
            self.ctl("%szone-flush" % params, wait)
