                                       ...
                                       const knot_dname_t *dname, size_t pos)
{
	...
	for (int i = 0; i < table->count; ++i) {
		if (table->dnames[i] == dname) {
			dbg_knot_response("Already present, skipping..\n");
			return;
		}
	}
...
static int knot_response_store_dname_pos(knot_compressed_dnames_t *table,
                                           ...
                                           int compr_cs)
{
dbg_knot_response_exec(
	char *name = knot_dname_to_str(dname);
	dbg_knot_response("Putting dname %s into compression table."
	...
	if (pos > KNOT_RESPONSE_MAX_PTR) {
		dbg_knot_response("Pointer larger than it can be, not"
		                      " saving\n");
...
dbg_knot_response_exec(
		...
		if (table->count == table->max &&
		    knot_response_realloc_compr(table) != 0) {
			dbg_knot_response("Unable to realloc.\n");
			return KNOT_ENOMEM;
		}

//		dbg_knot_response("Saving..\n");
...
{
	for (int i = 0; i < table->count; ++i) {
//		dbg_knot_response("Comparing dnames %p and %p\n",
//		                      dname, table->dnames[i]);
//dbg_knot_response_exec(
//		char *name = knot_dname_to_str(dname);
//		dbg_knot_response("(%s and ", name);
...
//);
		...
		if (ret == 0) {
			dbg_knot_response("Found offset: %zu\n",
			                      table->offsets[i]);
                                         ...
                                         uint8_t *wire, size_t max)
{
	...
	dbg_knot_response("Size of the dname with ptr: %d\n", size + 2);
...
dbg_knot_response_exec(
		...
		dbg_knot_response("Searching for name %s in the compression"
		...
		if (compr_cs || knot_dname_node(to_find, 1) == NULL
		    ...
		    || knot_node_parent(knot_dname_node(to_find, 1), 1)
		       == NULL) {
			dbg_knot_response("compr_cs: %d\n", compr_cs);
			dbg_knot_response("knot_dname_node(to_find, 1) == %p"
			                    "\n", knot_dname_node(to_find, 1));
			
			if (knot_dname_node(to_find, 1) != NULL) {
				dbg_knot_response("knot_node_owner(knot_dname_node("
						    "to_find, 1)) = %p, to_find = %p\n",
					   knot_node_owner(knot_dname_node(to_find, 1)),
					   to_find);
				dbg_knot_response("knot_node_parent(knot_dname_node("
						    "to_find, 1), 1) = %p\n",
				      knot_node_parent(knot_dname_node(to_find, 1), 1));
			}
			break;
			...
			to_find = knot_node_owner(
			     knot_node_parent(knot_dname_node(to_find, 1), 1));
			dbg_knot_response("New to_find: %p\n", to_find);
		}
#endif
	}
	...
	dbg_knot_response("Max size available for domain name: %zu\n", max);
	
	if (offset > 0) {  // found such dname somewhere in the packet
		dbg_knot_response("Found name in the compression table.\n");
		...
		dbg_knot_response("Not found, putting whole name.\n");
	...
	if (knot_response_store_dname_pos(compr->table, dname, not_matched,
	    ...
	    != 0) {
		dbg_knot_response("Compression info could not be stored."
		                      "\n");
	}
                                      ...
                                      int compr_cs)
{
	...
	dbg_knot_response("Max size: %zu, owner pos: %zu, owner size: %d\n",
	...
	dbg_knot_response("Owner position: %zu\n", compr->owner.pos);
	...
	if (compr->owner.pos == 0 || compr->owner.pos > KNOT_RESPONSE_MAX_PTR) {
		...
		*rrset_wire += compr->owner.size;
		...
		dbg_knot_response("Putting pointer: %zu\n",
		                      compr->owner.pos);
	...
	dbg_knot_response("Max size: %zu, size: %d\n", max_size, size);

	dbg_knot_response("Wire format:\n");

	// put rest of RR 'header'
	knot_wire_write_u16(*rrset_wire, rrset->type);
	dbg_knot_response("  Type: %u\n", rrset->type);
	dbg_knot_response("  Type in wire: ");
	dbg_knot_response_hex((char *)*rrset_wire, 2);
	*rrset_wire += 2;

	knot_wire_write_u16(*rrset_wire, rrset->rclass);
	dbg_knot_response("  Class: %u\n", rrset->rclass);
	dbg_knot_response("  Class in wire: ");
	dbg_knot_response_hex((char *)*rrset_wire, 2);
	*rrset_wire += 2;

	knot_wire_write_u32(*rrset_wire, rrset->ttl);
	dbg_knot_response("  TTL: %u\n", rrset->ttl);
	dbg_knot_response("  TTL in wire: ");
	dbg_knot_response_hex((char *)*rrset_wire, 4);
	*rrset_wire += 4;
	...
	*rrset_wire += 2;
	...
	for (int i = 0; i < rdata->count; ++i) {
		...
		case KNOT_RDATA_WF_COMPRESSED_DNAME: {
			...
			dbg_knot_response("Compressed dname size: %d\n",
			                      ret);
			*rrset_wire += ret;
			rdlength += ret;
			compr->wire_pos += ret;
		...
		case KNOT_RDATA_WF_LITERAL_DNAME: {
			...
			dbg_knot_response("Uncompressed dname size: %d\n",
			                      dname->size);
...
//			dbg_knot_response("Raw data size: %d\n",
//			                      raw_data[0] + 1);
//			*rrset_wire += raw_data[0];
...
dbg_knot_response_exec(
	...
	dbg_knot_response("Converting RRSet with owner %s, type %s\n",
	                      name, knot_rrtype_to_string(rrset->type));
	free(name);
	dbg_knot_response("  Size before: %zu\n", *size);
);
	...
	dbg_knot_response("    Owner size: %d, position: %zu\n",
	...
	do {
		...
		if (ret < 0) {
			...
			dbg_knot_response("Some RR didn't fit in.\n");
			return KNOT_ESPACE;
		}

		dbg_knot_response("RR of size %d added.\n", ret);
		rrset_size += ret;
		++rrs;
	...
	*size += rrset_size;
	...
	dbg_knot_response("  Size after: %zu\n", *size);
...
dbg_knot_response_exec(
	...
	if (rrs >= 0) {
		...
		resp->size += size;
		dbg_knot_response("RRset added, size: %zu, RRs: %d, total "
		                      "size of response: %zu\n\n", size, rrs,
		                      resp->size);
	} else if (tc) {
		dbg_knot_response("Setting TC bit.\n");
                            ...
                            int override_max_size)
{
	...
	if (override_max_size) {
		dbg_knot_response("Overriding max size to: %u\n",
		                    resp->opt_rr.payload);
                                     ...
                                     int check_duplicates, int compr_cs)
{
	...
	dbg_knot_response("add_rrset_answer()\n");
	...
	dbg_knot_response("Trying to add RRSet to Answer section.\n");
	dbg_knot_response("RRset: %p\n", rrset);
	dbg_knot_response("Owner: %p\n", rrset->owner);
                                        ...
                                        int check_duplicates, int compr_cs)
{
	...
	dbg_knot_response("Trying to add RRSet to Authority section.\n");
                                         ...
                                         int check_duplicates, int compr_cs)
{
	...
	dbg_knot_response("Trying to add RRSet to Additional section.\n");
