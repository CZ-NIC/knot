...
static void test_store_load(const knot_dname_t *apex)
{
	...
	int ret = journal_insert(jj, m_ch, 1);
...
static void test_merge(const knot_dname_t *apex)
{
	...
	for (i = 0; !merged_present() && i < 40000; i++) {
		ret = journal_insert(jj, tm_chs(apex, i), 1);
