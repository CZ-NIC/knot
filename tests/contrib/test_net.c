                               ...
                               int type, const char *name, const char *addr_name)
{
	...
	int client = net_connected_socket(type, server_addr, source_addr, NULL);
...
static void test_refused(void)
{
	...
	client = net_connected_socket(SOCK_STREAM, &addr, NULL, NULL);
...
static void test_nonblocking_mode(int type)
{
	...
	struct sockaddr_storage server_addr = addr_from_socket(server);
	client = net_connected_socket(type, &server_addr, NULL, NULL);
...
static void test_nonblocking_accept(void)
{
	...
	int client = net_connected_socket(SOCK_STREAM, &addr_server, NULL, NULL);
	...
	close(client);
	client = net_connected_socket(SOCK_STREAM, &addr_server, NULL, NULL);
