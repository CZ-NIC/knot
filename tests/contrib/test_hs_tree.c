...
static int hash_uint32_xor(uint8_t *res, uint8_t *childs[], unsigned nchilds, unsigned hash_len, void *ctx, int alg)
{
	*(uint32_t *)res = 0;
	for (unsigned i = 0; i < nchilds; i++) {
		*(uint32_t *)res ^= (*(const uint32_t *)childs[i]);
	}
	return 0;
}
...
static int check_sorted(hs_tree_t *t, hs_node_t *n, unsigned depth)
{
	...
	for (unsigned i = 0; i < n->size; i++) {
		uint32_t *h = (uint32_t *)(leaf_parent(t, depth) ? n->childs[i] : go_branch(t, n, i)->any_leaf);
		if (!leaf_parent(t, depth)) {
			(void)check_sorted(t, go_branch(t, n, i), depth + 1);
		}
		assert(last < be32toh(*h));
		last = be32toh(*h);
	}
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	hs_tree_t t = { .hash_len = 4, .width_4bits = 2, .depth = 3 };
	uint8_t first[4] = { 1, 2 };
	ok(0 == hs_tree_add(&t, first), "add first");
	ok(0 == hs_tree_hash(&t, hash_uint32_xor, NULL, &hash), "rehash ok");
	ok(memcmp(hash, first, t.hash_len) == 0, "first is whole");
	check_tree(&t);

	uint8_t second[4] = { 1, 3 };
