...
int main(int argc, char *argv[])
{
	...
	hr_tree_check(&t);
	ok(hr_tree_hash(&t)[1] == 0, "without first hash");

	ok(-ENOENT == hr_tree_rem(&t, first), "remove removed first");
	hr_tree_check(&t);

	t.rehash_cb = hash_error;
	ok(-1 == hr_tree_add(&t, first), "add with hash error");
